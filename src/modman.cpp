...
int i, j, num, sourcefile = 0, macro = 1, recolor_all = 0;
...
cmd( "button .bbar.wrap -image wrapImg -relief $bRlf -overrelief $ovBrlf -command { \
		if { $wrap == 0 } { \
			set wrap 1 \
		} { \
			set wrap 0 \
...
if ( ! macro )
{
	...
	cmd( ".v.i add command -label \"search(...)\" -command { set choice 5 5} -accelerator Ctrl+A" );
