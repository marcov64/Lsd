    ...
    if (abmat==1){
      for ( j = min_c; j <= max_c; ++j ) {
          if ( j >= start[ i ] && j <= end[ i ] && is_finite( data[ i ][ j ] ) ) {	// ignore NaNs
                    //  sprintf(msg,"\n %g ",data[i][j]);plog(msg);
                      if (i==0) series1.push_back(data[i][j]);
                      else if (i==1) series2.push_back(data[i][j]);
...
int sort_labels_down( const void* a, const void* b )
{
  ...
  if ( diff != 0 )
  { return diff; }
  else if ( ( ( store* ) a )->end != ( ( store* ) b)->end )
  { return ( ( store* ) b )->end - ( ( store* ) a )->end; }
  else if ( ( ( store* ) a)->start != ( ( store* ) b )->start )
  ...
  *choice = 0;
  ...
  for ( j = 0; j < block_length; ++j ) {
    ...
    for ( i = 0; i < nv; i += block_length )
      ...
      else if ( start[ i + j ] <= max_c && end[ i + j ] >= min_c && start[ i + j ] <= time_sel && end[ i + j ] >= time_sel )
  ...
  *choice = 0;
  ...
  for ( i = 0; i < nv; ++i ) {
    ...
    { bins[ j ].min = bins[ j ].max = data[ i ][ time_cs ]; }
      ...
      else if ( bins[ j ].max < data[ i ][ time_cs ] )
