			...
			case 30:
				...
				if ( choice == 3 || choice == 4 )
				{
					...
					for ( i = 0; i < j; ++i )
					{
						...
						{
							if ( std::isfinite( vs[ k ].data[ h - l ] ) )// ignore NaNs
							{
								r = 0;
								switch ( p )
								{
			...
			case 24:
				if ( choice == 24 )
				{
					...
					if ( vs.size( ) > 0 )
					{
						...
						cmd( "pack .da.s.i.c .da.s.i.a .da.s.i.e .da.s.i.f .da.s.i.m -anchor w" );
...
double gui::lower_bound( double a, double b, double marg, double marg_eq, int dig )
{
	...
	{
		if ( rmin == 0.0 )
			return sim.round_digits( - marg_eq, dig );
		else
			if ( rmin > 0 )
				return sim.round_digits( rmin * ( 1 - marg_eq ), dig );
			else
				return sim.round_digits( rmin * ( 1 + marg_eq ), dig );
	}

	if ( rmin == 0.0 )
		return sim.round_digits( - marg, dig );
	else
		if ( rmin > 0 )
			return sim.round_digits( rmin * ( 1 - marg ), dig );
		else
			return sim.round_digits( rmin * ( 1 + marg ), dig );
}
...
double gui::upper_bound( double a, double b, double marg, double marg_eq, int dig )
{
	...
	if ( rmin == rmax )
	{
		if ( rmax == 0.0 )
			...
			if ( rmax > 0 )
				return sim.round_digits( rmax * ( 1 + marg_eq ), dig );
			else
				return sim.round_digits( rmax * ( 1 - marg_eq ), dig );
	}
	...
	if ( rmax == 0.0 )
		...
		if ( rmax > 0 )
			return sim.round_digits( rmax * ( 1 + marg ), dig );
		else
			return sim.round_digits( rmax * ( 1 - marg ), dig );
}
...
void lsd::object::count_labels_mem( int *count, const char *lab )
{
	...
	for ( auto cv = v; cv != NULL; cv = cv->next )
		if ( ( lab == NULL && cv->attr->save ) || ( lab != NULL && ! strcmp( cv->attr->label, lab ) ) )
		{
			if ( ! cv->attr->save )
				found = true;
		...
		if ( ( lab == NULL && cv->attr->save ) || ( lab != NULL && da->disable && ! strcmp( cv->attr->label, lab ) ) )
		{
			...
			if ( da->disable )
			{
				...
				else
				{
					...
					if ( cv->data == NULL )
						cv->data = ( double * ) malloc( ( cv->attr->num_lag + 1 ) * sizeof( double ) );
...
bool gui::create_series( bool mc, str_vecT v_names )
{
	...
	for ( k = 0, l = vs.size( ) - new_series; k < new_series; ++k, ++l, ++var_num )
	{
		...
		cmd( "if { ! [ dict exists serDescrDict %s ] } { \
			...
			}", vs[ l ].label.c_str( ), vs[ l ].label.c_str( ), mc ? "Monte Carlo series" : "Created from other series" );

		cmd( "add_series \"%s %s (%d-%d) #%d\" %s", vs[ l ].label.c_str( ), vs[ l ].tag.c_str( ), vs[ l ].start, vs[ l ].end, vs[ l ].rank, vs[ l ].parent.c_str( ) );
...
bool gui::create_maverag( void )
{
	...
	for ( i = 0, l = ov; i < nv; ++i, ++l )
	{
		...
		cmd( "if { ! [ dict exists serDescrDict %s ] } { \
			...
			}", vs[ l ].label.c_str( ), vs[ l ].label.c_str( ), flt, str[ i ] );
