...
proc open_browser { dir fnb } {
	...
	switch -- $CurPlatform {
		linux { set error [ open_terminal $fqnb $html_browser ] }
		windows { set error [ open_terminal "start $html_browser $fqnb" ] }
		mac {
			set cmdline "osascript -e \"tell application \\\"$html_browser\\\" to open location \\\"$fqnb\\\"\""
			set cmdline [ concat $cmdline "-e \"tell application \\\"$html_browser\\\" to activate\"" ]
...
proc open_terminal { cmd { term "" } } {
	...
	if { $CurPlatform eq "mac" && [ string equal -nocase $term Terminal ] } {
		...
		set tclPath ""
		if { ! [ catch { exec brew --prefix tcl-tk@8 } tclPath ] && $tclPath ne "" } {
...
proc make_background { target threads term precompiled } {
	...
	if [ string equal $CurPlatform windows ] {
		...
		puts -nonewline $file "$DefaultMakeExe --jobs=$threads --file=makefile$makeSuffix 2> makemessage.txt\n"
		# --jobserver-style=pipe or =sem may be required in make 4.4
		close $file
