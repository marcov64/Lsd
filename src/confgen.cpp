...
int last_run = 1;			// simulation number running
...
int lsdmain( int argn, const char **argv )
{
	...
	else
	{
		for ( i = 1; i < argn; i += 2 )
		{
			...
			{
				conf_file = new char[ strlen( argv[ 1 + i ] ) + 1 ];
				strcpy( conf_file, argv[ 1 + i ] );
				continue;
			}
			...
			{
				conf_name = new char[ strlen( argv[ 1 + i ] ) + 1 ];
				strcpy( conf_name, argv[ 1 + i ] );
				continue;
			}
	...
	if ( config_file == NULL )
	{
		config_file = new char[ strlen( conf_file ) + 5 ];
		strcpy( config_file, conf_file );
	...
	if ( conf_name == NULL )
	{
		conf_name = new char[ strlen( conf_file ) + 1 ];
	...
	for ( i = 1; i <= confs; ++i )
	{
		...
		if ( ! save_xml_configuration( NULL, NULL, NULL, confs == 1 ? 0 : i, false, true ) )
		{
...
bool change_configuration( lsd::object *root, int findex )
{
	...
	for ( i = 0; i < num_vars; ++i )
	{
		...
		{
			last_t = ( int ) max( 0, round( values[ i ][ findex - 1 ] ) );
			continue;
		}
		if ( ! strcmp( vars[ i ], "_numRuns_" ) )
		{
			last_run = ( int ) max( 0, round( values[ i ][ findex - 1 ] ) );
			continue;
		}
...
Dummy for linking
*********************************/
double equation::_fun_( lsd::object * ) { return NAN; }
