...
v[3] = VS( PARENT, "_own2" ) == 1 ? VS( GRANDPARENT, "bG" ) :
	   VS( PARENT, "_postChg" ) ? VS( GRANDPARENT, "bChg" ) :
...
v[4] = VS( cur, "_p1" );						// machine FOB price

if ( GRANDPARENTS( cur ) != COUNTRL3 )			// foreign? use CIF price
	v[4] *= ( VS( COUNTRL3, "e" ) / VS( GRANDPARENTS( cur ), "e" ) ) /
			( ( 1 - VS( COUNTRL3, "trMK" ) ) *
			  ( 1 - VS( PARENTS( cur ), "trX1" ) ) );

if ( v[2] <= 0 || v[4] / VS( GRANDPARENT, "m2" ) / v[2] > v[3] )
...
h = VS( COUNTRL3, "flagWorkerLBU" );			// worker-level learning mode
...
CYCLE( cur, "WrkV" )
{
	v[3] += v[4] = VS( SHOOKS( cur ), "_edS" );	// add worker educ. + skills
...
if ( v[6] - v[5] > v[2] )						// is last worker in excess?
{
	...
	if ( v[6] < v[2] )							// makes sense to hire workers?
	{
		...
		{										// worker has public skills
			i = V( "__IDvint" );
			v[7] = V_EXTS( COUNTRL3, countryE, vintProd[ i ].sVp );
		}
		else
...
CYCLE( cur, "WrkV" )
{
	v[2] += VS( SHOOKS( cur ), "_edS" );		// add worker educ. + skills
	++i;
}
