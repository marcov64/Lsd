  ...
  for( k in 1 : nExp ){             # do for each experiment
    ...
    for( l in 1 : nSize ) {                        # for all MC samples (files)
      ...
      for( i in ( csBeg + 1 ) : ( csEnd + 1 ) ) {  # all time steps
        ...
        cols <- c( fShare, "A2d", "postChg" )
        fd <- t( mc[ i, cols, ] )
        fd.1 <- t( mc[ max( i - 1, 1 ), cols, ] )
        cols <- c( "f2", "A2", "postChg" )
        colnames( fd ) <- cols
        colnames( fd.1 ) <- cols
        ...
        prdDec[[ k ]][[ l ]]$A2.pre.1[ j ] <- A2.pre.1 <-
          sum( fd$f2.1[ fd$postChg == 0 ] * fd$A2.1[ fd$postChg == 0 ],
               na.rm = TRUE )
        prdDec[[ k ]][[ l ]]$A2.pre[ j ] <- A2.pre <-
          sum( fd$f2[ fd$postChg == 0 ] * fd$A2[ fd$postChg == 0 ], na.rm = TRUE )
        prdDec[[ k ]][[ l ]]$delta.A2.pre[ j ] <- ( A2.pre - A2.pre.1 ) / A2.1

        # post-change firms
        prdDec[[ k ]][[ l ]]$A2.pos.1[ j ] <- A2.pos.1 <-
          sum( fd$f2.1[ fd$postChg == 1 ] * fd$A2.1[ fd$postChg == 1 ], na.rm = TRUE )
        prdDec[[ k ]][[ l ]]$A2.pos[ j ] <- A2.pos <-
          sum( fd$f2[ fd$postChg == 1 ] * fd$A2[ fd$postChg == 1 ], na.rm = TRUE )
        ...
        prdDec[[ k ]][[ l ]]$A2.inc.pre.1[ j ] <- A2.inc.pre.1 <-
          sum( fd$f2.1[ fd$postChg == 0 & fd$type == 2 ] *
                 fd$A2.1[ fd$postChg == 0 & fd$type == 2 ], na.rm = TRUE )
        prdDec[[ k ]][[ l ]]$A2.inc.pre[ j ] <- A2.inc.pre <-
          sum( fd$f2[ fd$postChg == 0 & fd$type == 2 ] *
                 fd$A2[ fd$postChg == 0 & fd$type == 2 ], na.rm = TRUE )
        ...
        prdDec[[ k ]][[ l ]]$A2.inc.pos.1[ j ] <- A2.inc.pos.1 <-
          sum( fd$f2.1[ fd$postChg == 1 & fd$type == 2 ] *
                 fd$A2.1[ fd$postChg == 1 & fd$type == 2 ], na.rm = TRUE )
        prdDec[[ k ]][[ l ]]$A2.inc.pos[ j ] <- A2.inc.pos <-
          sum( fd$f2[ fd$postChg == 1 & fd$type == 2 ] *
                 fd$A2[ fd$postChg == 1 & fd$type == 2 ], na.rm = TRUE )
        ...
        prdDec[[ k ]][[ l ]]$W.pre[ j ] <-          # pre-change firms
          sum( fd$f2.1[ fd$type == 2 & fd$postChg == 0 ] *
                 fd$delta.A2[ fd$type == 2 & fd$postChg == 0 ],
               na.rm = TRUE ) / A2.1
        prdDec[[ k ]][[ l ]]$W.pos[ j ] <-          # post-change firms
          sum( fd$f2.1[ fd$type == 2 & fd$postChg == 1 ] *
                 fd$delta.A2[ fd$type == 2 & fd$postChg == 1 ],
               na.rm = TRUE ) / A2.1

        # incumbents between components
        prdDec[[ k ]][[ l ]]$B.pre[ j ] <-
          sum( fd$A2.1[ fd$type == 2 & fd$postChg == 0 ] *
                 fd$delta.f2[ fd$type == 2 & fd$postChg == 0 ],
               na.rm = TRUE ) / A2.1
        prdDec[[ k ]][[ l ]]$B.pos[ j ] <-
          sum( fd$A2.1[ fd$type == 2 & fd$postChg == 1 ] *
                 fd$delta.f2[ fd$type == 2 & fd$postChg == 1 ],
               na.rm = TRUE ) / A2.1
