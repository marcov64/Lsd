...
double Bda = VS( MACSTAL1, "Bda" );
double Bfail = VS( MACSTAL1, "Bfail" );
double BadDeb = VS( MACSTAL1, "BadDeb" );
double HHb = VS( MACSTAL1, "HHb" );
double HPb = VS( MACSTAL1, "HPb" );
...
objVecT EmErr, Kerr, Lerr, Oerr, Qerr, cErr, perr, muerr;
int errors = 0;									// error counter
h = j = k = v[4] = v[5] = v[6] = v[7] = v[8] = v[9] = 0;// accumulators
...
CYCLES( ENESECL1, cur, "FirmE" )
{
	...
	CYCLES( cur, cur1, "Dirty" )
	{
		...
		if ( VS( cur1, "__Qde" ) > VS( cur1, "__Kde" ) || VS( cur1, "__Qde" ) < 0 ||
			 ( VS( cur1, "__Qde" ) > 0 && VS( cur1, "__EmDE" ) == 0 ) )
			Qerr.push_back( cur1 );
	...
	CYCLES( cur, cur1, "Green" )
	{
		...
		if ( VS( cur1, "__Qge" ) > VS( cur1, "__Kge" ) || VS( cur1, "__Qge" ) < 0 )
			Qerr.push_back( cur1 );
	...
	if ( ( VS( cur, "_Qe" ) > 0 && VS( cur, "_Le" ) - VS( cur, "_LeRD" ) <= 0 ) ||
		 ( VS( cur, "_Qe" ) <= 0 && VS( cur, "_Le" ) - VS( cur, "_LeRD" ) > TOL ) )
		Lerr.push_back( cur );
...
double SfAcc = VS( ENESECL1, "SfAcc" );
...
double entryE = VS( ENESTAL1, "entryE" );
double exitE = VS( ENESTAL1, "exitE" );
...
double nonNeg[ ] = { CDe, CDeC, Ce, CSe, DebE, DebEmax, DivE, EIe, EmE, EnGDP, Fe, 
					 HHe, HPe, Ie, JOe, Ke, Kde, Kge, Le, LeD, LeDrd, LeRD, Qe, 
					 RDe, RSe, SIe, SIeD, Se, TaxE, We, entryE, exitE, exitEfail, 
					 fGE, fKge, innDE, innGE };
double posit[ ] = { Cf, AtauDEavg, De, ICtauGEavg, QeO, SfAcc, ageEavg, 
					emTauDEavg, muEavg, pE };
...
LOG( "\n   E fGE=%.2g fKge=%.2g Ie=%.3g SIeD=%.3g SIe=%.3g EIe=%.3g", 
	 fGE, fKge, Ie, SIeD, SIe, EIe );

check_error( Kde < floor( v[5] ) || Kge < floor( v[7] ) || 
			 round( Ke ) != round( Kde + Kge ) || floor( SIeD ) > ceil( Ke ), 
			 "INCONSISTENT-CAPITAL", 0, & errors );

check_error( ceil( SIeD ) < floor( SIe ) || round( SIe + EIe ) != round( Ie ), 
...
check_error( Qerr.size( ) > 0, "BAD-GEN-PLANT", Qerr.size( ), & errors ); 
...
check_error( floor( Qe ) > Ke || round( Qe ) > round( De ) ||
			 ceil( Qe ) < v[6] + v[8], 
...
check_error( LeD < JOe || Le > LeD || LeDrd > LeD || LeRD > Le || Le > Ls, 
...
LOG( "\n   E We=%g Cf=%.3g Ce=%.3g TaxE=%.3g PiE=%.3g DivE=%.3g", 
	 We, Cf, Ce, TaxE, PiE, DivE );

check_error( RDe > We || floor( Cf + We ) > Ce || We + PiE > Se || TaxE > Se || 
			 PiE > Se, "INCONSISTENT-COST", 0, & errors );
...
check_error( muerr.size( ) > 0, "BAD-MARKUP-FIRMS", muerr.size( ), & errors ); 
...
check_error( HHe > 1 || HPe > 2, "INCONSISTENT-STATS", 0, & errors );
...
CYCLES( ENESECL1, cur, "FirmE" )
{
	...
	CYCLES( cur, cur1, "Dirty" )
	{
		...
		if ( ( VS( cur1, "__Qde" ) == 0 && VS( cur1, "__LdeD" ) > 0 ) || 
			 ( VS( cur1, "__Qde" ) > 0 && VS( cur1, "__LdeD" ) == 0 ) ) 
			Lerr.push_back( cur1 );
	}
	...
	CYCLES( cur, cur1, "Green" )
	{
		...
		if ( ( VS( cur1, "__Qge" ) == 0 && VS( cur1, "__LgeD" ) > 0 ) || 
			 ( VS( cur1, "__Qge" ) > 0 && VS( cur1, "__LgeD" ) == 0 ) ) 
			Lerr.push_back( cur1 );
	}
	...
	double nonNeg[ ] = { _CDe, _CDeC, _CSe, _Ce, _Cf, _De, _DebE, _DebEmax, 
						 _DivE, _EIe, _EmE, _JOe, _Ke, _Kde, _Kge, _Le, _LeD, 
						 _LeDrd, _LeRD, _Qe, _Qde, _Qge, _QeO, _RDe, _Se, _SIe, 
						 _SIeD, _SIdeD, _SIgeD, _TaxE, _We, _fKge, _innDE, 
						 _innGE, _tEent };
	double posit[ ] = { _muE, _AtauDE, _ICtauGE, _emTauDE, _fE, _pE };
	...
	check_error( v[18] + v[19] == 0, "NO-PLANTS", 0, & errors ); 
	...
	check_error( _RDe <= 0, "NO-R&D", 0, & errors );
	...
	LOG( "\n   e muE=%.2g pE=%.2g Se=%g We=%g Ce=%.3g Cf=%.3g fE=%.2g", _muE, _pE, 
		 round( _Se ), round( _We ), _Ce, _Cf, _fE );
	fprintf( firmsE, ",%g,%g,%g,%g,%g,%g,%g", _muE, _pE, _Se, _We, _Ce, _Cf, _fE );
		  
	check_error( _RDe > _We || floor( _Cf + _We ) > _Ce || _We + _PiE > _Se ||
				 _TaxE > _Se || _PiE > _Se, "INCONSISTENT-COST", 0, & errors );
...
if ( T == v[2] )
{
	LOG( "\n eee TESTING OF ENERGY FIRMS FINISHED" );
	fclose( firmsE );
	firmsE = NULL;
}
