...
if ( VS( GRANDPARENT, "flagCreditRule" ) > 0 )
	v[0] = ( 1 - VS( PARENT, "phi1" ) - VS( PARENT, "phi2" ) ) * 
		   max( 0, V( "_TC" ) - VL( "_Loans", 1 ) );
else
	v[0] = -1;									// no limit
...
if ( VS( GRANDPARENT, "flagCreditRule" ) > 0 )
	v[0] = VS( PARENT, "phi1" ) * max( 0, V( "_TC" ) - VL( "_Loans", 1 ) );
else
	v[0] = -1;									// no limit
...
if ( VS( GRANDPARENT, "flagCreditRule" ) > 0 )
	v[0] = VS( PARENT, "phi2" ) * max( 0, V( "_TC" ) - VL( "_Loans", 1 ) );
else
	v[0] = -1;									// no limit
...
CYCLE( cur, "CliE" )							// sector 1
{
	j = VLS( SHOOKS( cur ), "_qcE", 1 );		// firm credit class
	v[4] = VLS( SHOOKS( cur ), "_DebE", 1 );	// firm debt
