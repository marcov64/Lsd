...
VS( ENESECL1, "entryEexit" );					// ensure entry-exit done
VS( CAPSECL1, "entry1exit" );
VS( CONSECL1, "entry2exit" );

firmRank firmData;
firmLisT rankE, rank1, rank2;

CYCLES( ENESECL1, cur, "FirmE" )				// build ranking for energy sec.
{
	v[1] = VS( cur, "_NWe" );					// net worth
	v[2] = VS( cur, "_Se" );					// sales
...
h = i = rankE.size( );							// # of firms in energy sector
j = 0; 											// firm counter
for ( firmLisT::iterator itr = rankE.begin( ); itr != rankE.end( ); ++j, ++itr )
{
	// find the credit class of firm
	k = ( j < i * 0.25 ) ? 1 : ( j < i * 0.5 ) ? 2 : ( j < i * 0.75 ) ? 3 : 4; 
	WRITES( itr->firm, "_qcE", k );				// save credit class
	WRITES( itr->firm, "_pOrdE", j + 1 );		// save pecking order
}
