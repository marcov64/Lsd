...
time_plots <- function( mcData, Pdata, mdata, Mdata, Sdata, cdata, Cdata,
  ...
  # add the series to dataset
  for( k in 1 : nExp ) {
    Pdata[[ k ]][ "WGDP" ] <- ( Pdata[[ k ]]$We + Pdata[[ k ]]$W1 + Pdata[[ k ]]$W2 ) /
      Pdata[[ k ]]$GDPnom
  ...
  plot_lists( c( "innDE", "innGE" ), Pdata, mdata, Mdata, cdata, Cdata, leg = legends,
              mask = TmaskPlot, nMC = nSize, CI = CI, col = colors,
              lty = lTypes, xlab = "Time",
  ...
  plot_lists( c( "HHe", "HH1", "HH2" ), Pdata, mdata, Mdata, cdata, Cdata, leg = legends,
              mask = TmaskPlot, nMC = nSize, CI = CI, col = colors,
              lty = lTypes, xlab = "Time",
  ...
  for( k in 1 : nExp ) {
    ...
    Pdata[[ k ]][ "nEntTe" ] <- colMeans( nEntTe, na.rm = TRUE )
  ...
  plot_lists( c( "nEntTe", "nEntT1", "nEntT2" ), Pdata, mdata, Mdata, sdMC = Sdata,
