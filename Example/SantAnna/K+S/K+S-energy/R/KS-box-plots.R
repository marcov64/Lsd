...
# !diagnostics suppress = plot_bxp_vio
...
box_plots <- function( mcData, mcStat, nExp, nSize, TmaxStat, TmaskStat,
  ...
  for( k in 1 : nExp ) {
    ...
    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "innDE", ],
                                        na.rm = TRUE ),
    ...
    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "HHe", ],
                                        na.rm = TRUE ),
                     tit = "Energy market concentration",
                     ylab = "Standardized Herfindahl-Hirschman index in energy sector" )

    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "HH2", ],
                                        na.rm = TRUE ),
                     tit = "Consumption market concentration",
                     ylab = "Standardized Herfindahl-Hirschman index in consumption-good sector" )

    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "muEavg", ],
                                        na.rm = TRUE ),
                     tit = "Energy mark-up",
                     ylab = "Weighted average mark-up amount in energy sector" )

    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "mu2avg", ],
                                        na.rm = TRUE ),
                     tit = "Consumption-good mark-up",
                     ylab = "Weighted average mark-up rate in consumption-good sector" )

    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "entryEexit", ] +
                                          mcData[[ k ]][ TmaskStat, "entry1exit", ] +
    ...
    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "EnGDP", ],
                                        na.rm = TRUE ),
                     tit = "Energy demand",
                     ylab = "Energy demand over GDP" )

    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "Em", ],
                                        na.rm = TRUE ),
                     tit = "CO2 emissions",
                     ylab = "CO2 emissions in kton" )

    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "CO2a", ],
                                        na.rm = TRUE ),
                     tit = "CO2 in atmosphere",
                     ylab = "CO2 atmospheric concentration in PPM" )

    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "Tm", ],
                                        na.rm = TRUE ),
                     tit = "Temperature anomaly",
                     ylab = "Temperature change in C from preindustrial reference" )

    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "shockAavg", ],
                                        na.rm = TRUE ),
                     tit = "Climate shocks (disaster generating function)",
                     ylab = "Expected climate shock size" )

    stat <- addStat( stat, k, colMeans( mcData[[ k ]][ TmaskStat, "fGE", ],
                                        na.rm = TRUE ),
                     tit = "Green energy share",
                     ylab = "Share of green plants on total generation" )
  }
