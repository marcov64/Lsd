...
// handle entrant or too few data?
if ( h > 0 && h >= T - 1 - j )
	END_EQUATION( CURRENT );

// sustain expectation during simulation startup (capital stock 50% renewal)
if ( T <= VS( PARENT, "eta" ) / 2 )
	END_EQUATION( CURRENT * ( 1 + VS( GRANDPARENT, "dGDP0" ) ) );
...
v[4] = max( floor( v[2] / v[3] ) - ceil( v[1] / v[3] ), 0 );// unused machines

// scan all vintages, from oldest to newest, preferring to use newer ones
v[0] = v[5] = v[6] = v[7] = v[8] = 0;			// accumulators
CYCLE( cur, "Vint" )							// choose vintages to use
{
	v[5] += v[9] = VS( cur, "__nVint" );		// number of machines in vintage
	v[0] += v[9] * VS( cur, "__cVint" );		// add machines cost
	v[6] += v[9] * VS( cur, "__AlpVint" );		// add labor productivities
	v[7] += v[9] * VS( cur, "__AeeVint" );		// add energy efficiencies
	v[8] += v[9] * VS( cur, "__AefVint" );		// add carbon taxes

	if ( v[4] >= v[9] )							// none to be used in the vint.?
	{
		v[4] -= v[9];							// less machines not to use
		v[10] = 0;								// no machines to use in vintage
	}
	else
	{
		v[10] = v[9] - v[4];					// machines to use in vintage
		v[4] = 0;								// no more machine not to use
	}
...
if ( v[5] == 0 )								// no machine?
{
	...
	v[5] = 1;									// 1 notional machine
	v[0] = VS( cur1, "_cTau" );					// machine unit cost
	v[6] = VS( cur1, "_AtauLP" );				// new machines productivity
	v[7] = VS( cur1, "_AtauEE" );				// new machines efficiency
	v[8] = VS( cur1, "_AtauEF" );				// new machines friendliness
}

WRITE( "_A2", v[6] / v[5] );					// potential labor productivity
WRITE( "_A2ee", v[7] / v[5] );					// potential energy efficiency
WRITE( "_A2ef", v[8] / v[5] );					// potential envir. friendliness
...
EQUATION( "_C2" )
/*
Operating costs of firm in consumption-good sector
*/
RESULT( V( "_W2" ) +							// labor effective total cost
		V( "_En2" ) * VS( ENESECL2, "pE" ) +	// energy total cost
		V( "_Em2" ) * VS( GRANDPARENT, "trCO2" ) )// carbon emission total tax
...
EQUATION( "_Em2" )
/*
CO2 (carbon) emissions produced by firm in consumption-good sector
*/
RESULT( SUM( "__EmVint" ) )
...
EQUATION( "_En2" )
/*
Energy consumed by firm in consumption-good sector
*/
RESULT( SUM( "__EnVint" ) )
...
v[5] = VL( "_K", 1 );							// operating capital stock
if ( ( int ) VS( GRANDPARENT, "flagClimShocks" ) & 4 )
{
	v[4] = 1 - VS( CLIMATL2, "shockA" );		// capital stock shock size

	if ( v[4] < 1 )								// shock to apply?
	{
		v[5] = 0;								// remaining K after shock
		CYCLE( cur, "Vint" )
			v[5] += WRITES( cur, "__nVint", ceil( v[4] * VS( cur, "__nVint" ) ) );
	}

}

v[5] = max( v[5] + v[3] - V( "_Kd" ), 0 );		// desired K shrinkage
...
// apply disaster generating function shock if enabled
if ( ( int ) VS( GRANDPARENT, "flagClimShocks" ) & 8 )
	v[0] *= 1 - VS( CLIMATL2, "shockA" );		// apply shock
...
EQUATION_DUMMY( "_A2ee", "" )
/*
Machine-level average energy efficiency of firm in consumption-good sector
Updated in '_c2'
*/

EQUATION_DUMMY( "_A2ef", "" )
/*
Machine-level average environmental friendliness of firm in consumption-good sector
Updated in '_c2'
*/
