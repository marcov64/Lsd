...
if ( v[0] < 0 && VS( COUNTRL2, "flagCreditRule" ) == 2 )
{
...
k = VS( COUNTRL2, "flagCreditRule" );			// credit limit & bail-out rule
...
if ( VS( COUNTRL2, "flagCreditRule" ) > 0 )
{
...
v[0] = V( "_fD" ) * VS( LABSUPL2, "SavAcc" );	// workers deposits
...
*/
RESULT( max( V( "_PiB" ) * VS( COUNTRL2, "tr" ), 0 ) )
...
v[0] = V( "_fD" ) * VLS( LABSUPL2, "SavAcc", 1 ) * VLS( PARENT, "rD", 1 );
												// workers deposits
...
CYCLE( cur, "Cli1" )							// rank sector 1 clients
{
	...
	v[2] = VLS( SHOOKS( cur ), "_S1l", 1 ) +
		   VLS( SHOOKS( cur ), "_S1x", 1 );		// total sales
...
CYCLE( cur, "Cli2" )							// rank sector 2 clients
{
	...
	v[2] = VLS( SHOOKS( cur ), "_S2l", 1 ) +
		   VLS( SHOOKS( cur ), "_S2x", 1 );		// total sales
	firmData.NWtoS = ( v[1] > 0 && v[2] > 0 ) ? v[1] / v[2] : 0;
	firmData.firm = SHOOKS( cur );
