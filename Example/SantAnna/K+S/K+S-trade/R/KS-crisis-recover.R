...
crisis_recover <- function( mcData, nExp, nSize, TmaskStat, warmUpPlot,
    ...
    for( j in 1 : nSize ) {       # for each MC case
      ...
      for( i in TmaskStat ) {
        if( recovery ) {                         # in a recovery?
          ...
          # recovery is over?
          if( log( mcData[[ k ]][ i, "Yreal", j ] ) >= preCrisisY ) {
            recovery <- FALSE
            ...
            # update statistics
            gap <- preCrisisY - log( mcData[[ k ]][ i, "Yreal", j ] )
            depthCrisis <- max( depthCrisis, gap )
            costCrisis <- costCrisis + gap
          }
        }

        if( ! recovery && mcData[[ k ]][ i, "dY", j ] < crisisTh &&
            i > crisisPre ) {
          recovery <- TRUE
          timeCrisis <- i
          depthCrisis <- costCrisis <- 0
          preCrisisY <- log( mean( mcData[[ k ]][ ( i - crisisPre ) : ( i - 1 ),
                                                  "Yreal", j ] ) )
          gTrend <- mean( growthTrend[ ( i - crisisPre ) : ( i - 1 ) ],
                          na.rm = TRUE )
          depthCrisis <- costCrisis <- preCrisisY -
            log( mcData[[ k ]][ i, "Yreal", j ] )
        }
      }
  ...
  for( k in 1 : nExp ) {                                # for each experiment
    ...
    plot_recovery( mcData[[ k ]][ , "Yreal", j ],
                   mcData[[ k ]][ , "dY", j ],
