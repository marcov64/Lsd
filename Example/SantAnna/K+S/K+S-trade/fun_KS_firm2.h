...
EQUATION( "_D2e" )
/*
Total demand expectation (in product units) of consumption-good firm
*/
...
k = VS( COUNTRL2, "flagExpect" );				// expectation form
...
switch ( k )
{
	...
	case 4:
		...
		v[0] = ( 1 + VS( PARENT, "e7" ) * ( v[1] - v[2] ) / v[2] +
				 VS( PARENT, "e8" ) * VLS( COUNTRL2, "dY", 1 ) ) * v[1];
		break;
}
...
v[1] = V( "_own2" ) == 1 ? VS( PARENT, "iotaG" ) : VS( PARENT, "iota" );
RESULT( max( ( 1 + v[1] ) * V( "_D2e" ) - VL( "_N", 1 ), 0 ) /
		VS( PARENT, "u" ) )
...
v[1] = V( "_own2" ) == 1 ? VS( PARENT, "iotaG" ) : VS( PARENT, "iota" );
v[2] = max( ( 1 + v[1] ) * V( "_D2e" ) - VL( "_N", 1 ), 0 );

// limited to the available capital stock
RESULT( min( v[2], VL( "_K", 1 ) ) )
...
EQUATION( "_Tax2" )
/*
Tax on profits paid by firm in consumption-good sector
Also updates '_NW2', '_Deb2', _CD2', '_CD2c', '_CS2'
*/
...
h = VS( COUNTRL2, "flagWorkerLBU" );			// worker-level learning mode
...
int fRule = V( "_postChg" ) ? VS( COUNTRL2, "flagFireRuleChg" ) :
							  VS( COUNTRL2, "flagFireRule" );
switch ( fRule )
{
...
k = VS( COUNTRL2, "flagTradeK" );				// international trade mode
v[1] = VS( PARENT, "m2" );						// machine modularity
v[2] = V( "_own2" ) == 1 ? VS( PARENT, "bG" ) : // required payback period
	   V( "_postChg" ) ? VS( PARENT, "bChg" ) : VS( PARENT, "b" );
...
CYCLE( cur, "Broch" )							// use brochures to find supplier
{
	...
	if ( k == 0 && GRANDPARENTS( cur1 ) != COUNTRL2 )// no intl trade?
		continue;

	// compare total machine unit cost (acquisition + operation for payback period)
	v[5] = VS( cur1, "_p1" );					// machine FOB price

	if ( GRANDPARENTS( cur1 ) != COUNTRL2 )		// foreign? use CIF price
		v[5] *= ( VS( COUNTRL2, "e" ) / VS( GRANDPARENTS( cur1 ), "e" ) ) /
				( ( 1 - VS( COUNTRL2, "trMK" ) ) *
				  ( 1 - VS( PARENTS( cur1 ), "trX1" ) ) );

	v[6] = v[5] / v[1] + v[2] * v[3] / VS( cur1, "_Atau" );

	if ( v[6] < v[4] )							// best so far?
	{
		v[4] = v[6];							// save current best supplier
...
EQUATION( "_w2o1" )
/*
Wage offer to category 1 workers in queue of firm in consumption-good sector
*/
RESULT( CFUN( wage_offer, 1 ) )


EQUATION( "_w2o2" )
/*
Wage offer to category 2 workers in queue of firm in consumption-good sector
*/
RESULT( CFUN( wage_offer, 2 ) )


EQUATION( "_w2o3" )
/*
Wage offer to category 3 workers in queue of firm in consumption-good sector
*/
RESULT( CFUN( wage_offer, 3 ) )
...
if ( k == T && v[1] > 0 )
{
	cur1 = PARENTS( SHOOKS( cur ) );			// supplier firm object
...
EQUATION( "_D2d" )
/*
Desired total demand (in product units) for firm in consumption-good sector
*/
VS( PARENT, "CPI" );							// ensure m.s. updated
RESULT( V( "_f2" ) * ( VS( PARENT, "D2lD" ) + VS( PARENT, "D2xD" ) ) )


EQUATION( "_M2" )
/*
Machine imports (in domestic currency) of firm in consumption-good sector
*/

k = VS( COUNTRL2, "IDcnt" );					// firm country
v[0] = V( "_Inom" );							// investment in period

if ( v[0] > 0 && CNT( VS( HOOK( TOPVINT ), "__IDvint" ) ) == ( ullongT ) k )
	v[0] = 0;									// ignore domestic investment
...
EQUATION( "_JO21" )
/*
Open category 1 job positions of firm in consumption-good sector
*/
RESULT( CFUN( open_positions, 1 ) )


EQUATION( "_JO22" )
/*
Open category 2 job positions of firm in consumption-good sector
*/
RESULT( CFUN( open_positions, 2 ) )


EQUATION( "_JO23" )
/*
Open category 3 job positions of firm in consumption-good sector
*/
RESULT( CFUN( open_positions, 3 ) )


EQUATION( "_K" )
/*
Capital employed by firm in consumption-good sector
Also updates '_oldVint'
*/
...
*/
RESULT( V( "_L21" ) + V( "_L22" ) + V( "_L23" ) )


EQUATION( "_L21" )
/*
Effective number of category 1 workers of firm in consumption-good sector
*/
VS( PARENT, "hires2" );							// make sure hiring done
RESULT( CFUN( count_workers, 1 ) )


EQUATION( "_L22" )
/*
Effective number of category 2 workers of firm in consumption-good sector
*/
VS( PARENT, "hires2" );							// make sure hiring done
RESULT( CFUN( count_workers, 2 ) )


EQUATION( "_L23" )
/*
Effective number of category 3 workers of firm in consumption-good sector
*/
VS( PARENT, "hires2" );							// make sure hiring done
RESULT( CFUN( count_workers, 3 ) )


EQUATION( "_L2d1" )
/*
Labor category 1 demand of firm in consumption-good sector
*/
RESULT( V( "_life2cycle" ) > 0 ?
		ceil( V( "_Q2" ) / V( "_A2" ) ) - V( "_L2d2" ) - V( "_L2d3" ) : 0 )


EQUATION( "_L2d2" )
/*
Labor category 2 demand of firm in consumption-good sector
*/
v[1] = V( "_own2" ) == 1 ? VS( LABSUPL2, "theta2g" ) : VS( LABSUPL2, "theta2" );
RESULT( VS( COUNTRL2, "flagEduc" ) != 0 && V( "_life2cycle" ) > 0 ?
		ceil( v[1] * V( "_Q2" ) / V( "_A2" ) ) : 0 )


EQUATION( "_L2d3" )
/*
Labor category 3 demand of firm in consumption-good sector
*/
v[1] = V( "_own2" ) == 1 ? VS( LABSUPL2, "theta3g" ) : VS( LABSUPL2, "theta3" );
RESULT( VS( COUNTRL2, "flagEduc" ) != 0 && V( "_life2cycle" ) > 0 ?
		ceil( v[1] * V( "_Q2" ) / V( "_A2" ) ) : 0 )
...
v[2] = V( "_JO21" ) + V( "_JO22" ) + V( "_JO23" );// current open positions
...
RESULT( ROUND( CURRENT + V( "_Q2e" ) - V( "_D2l" ) - V( "_D2x" ), 0, 0.001 ) )
...
*/
RESULT( V( "_S2l" ) + V( "_S2x" ) + V( "_iD2" ) - V( "_W2" ) - V( "_i2" ) )
...
EQUATION( "_S2d" )
/*
Desired sales of firm in consumption-good sector
*/
RESULT( V( "_p2" ) * ( V( "_Q2e" ) + VL( "_N", 1 ) ) )


EQUATION( "_S2l" )
/*
Local sales of firm in consumption-good sector
*/
VS( PARENT, "D2" );								// ensure demand is allocated
RESULT( V( "_p2" ) * V( "_D2l" ) )
...
EQUATION( "_S2x" )
/*
Export sales of firm in consumption-good sector
*/
VS( PARENT, "D2" );								// ensure demand is allocated
RESULT( V( "_p2" ) * V( "_D2x" ) )
...
EQUATION( "_TaxM2" )
/*
Local import duties (in domestic currency) paid by firm in consumption-good sector
*/
...
EQUATION( "_TaxM2f" )
/*
Foreign import duties (in domestic currency) paid by firm in consumption-good sector
*/
RESULT( V( "_X2" ) - V( "_S2x" ) - V( "_TaxX2" ) )


EQUATION( "_TaxX2" )
/*
Local export duties (in domestic currency) paid by firm in consumption-good sector
*/
RESULT( V( "_S2x" ) * ( 1 / ( 1 - VS( PARENT, "trX2" ) ) - 1 ) )
...
EQUATION( "_X2" )
/*
Exports (in domestic currency including duties) of firm in consumption-good sector
*/
RESULT( ( V( "_S2x" ) + V( "_TaxX2" ) ) / ( 1 - VS( WORLDL2, "trMCavgW" ) ) )
...
*/
RESULT( CFUN( mov_avg_bound, "_A2", VS( COUNTRL2, "mLim" ),
		VS( COUNTRL2, "mPer" ) ) )
...
if ( VS( COUNTRL2, "flagWorkerLBU" ) == 0 )		// no learning ?
	END_EQUATION( INISKILL );
...
EQUATION_DUMMY( "_D2l", "" )
/*
Local demand (in product units) fulfilled by firm in consumption-good sector
Updated in 'D2'
*/

EQUATION_DUMMY( "_D2x", "" )
/*
Export demand (in product units) fulfilled by firm in consumption-good sector
Updated in 'D2'
*/
