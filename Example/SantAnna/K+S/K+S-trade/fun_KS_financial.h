...
v[1] = VS( COUNTRL1, "Def" );					// public deficit
...
v[0] = CURRENT;									// last period rate
v[1] = V( "rAdj" );								// rate adjustment step
v[2] = VLS( COUNTRL1, "DebY", 1 );				// public debt over GDP
...
EQUATION( "IntRes" )
/*
International reserves (foreign cash) hold by central bank
*/
RESULT( CURRENT + VS( COUNTRL1, "X" ) - VS( COUNTRL1, "M" ) +
		VS( COUNTRL1, "TaxM" ) - VS( COUNTRL1, "TaxMf" ) )
...
// clear vectors
EXEC_EXTS( COUNTRL1, countryE, bankPtr, clear );
EXEC_EXTS( COUNTRL1, countryE, bankWgtd, clear );
...
CYCLE( cur, "Bank" )
{
	EXEC_EXTS( COUNTRL1, countryE, bankPtr, push_back, cur );// pointer to bank

	v[1] += v[2] = max( VS( cur, "_fD" ), 0 );
	EXEC_EXTS( COUNTRL1, countryE, bankWgtd, push_back, v[2] );
...
for ( j = 0; j < i; ++j )
{
	v[0] += V_EXTS( COUNTRL1, countryE, bankWgtd[ j ] ) / v[1];
	WRITE_EXTS( COUNTRL1, countryE, bankWgtd[ j ], min( v[0], 1 ) );
}
...
CYCLES( CAPSECL1, cur, "Firm1" )				// rank entire sector 1
{
	...
	v[2] = VLS( cur, "_S1l", 1 ) + VLS( cur, "_S1x", 1 );// total sales
...
CYCLES( CONSECL1, cur, "Firm2" )				// rank entire sector 2
{
	...
	v[2] = VLS( cur, "_S2l", 1 ) + VLS( cur, "_S2x", 1 );// total sales
	firmData.NWtoS = ( v[1] > 0 && v[2] > 0 ) ? v[1] / v[2] : 0;
	firmData.firm = cur;
...
dblVecT *weight = & V_EXTS( COUNTRL1, countryE, bankWgtd );// bank weights
