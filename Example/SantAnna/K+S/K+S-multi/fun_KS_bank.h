...
if ( VS( GRANDPARENT, "flagCreditRule" ) == 1 )
{
	v[1] = SUMS( GRANDPARENT, "F1" );			// # firms in sector 1

	// credit allocation for sector 1
	v[2] = v[1] / ( v[1] + SUMS( GRANDPARENT, "F2" ) );
...
if ( VS( GRANDPARENT, "flagCreditRule" ) == 1 )
{
	v[1] = SUMS( GRANDPARENT, "F2" );			// # firms in sector 2

	// credit allocation for sector 2
	v[2] = v[1] / ( SUMS( GRANDPARENT, "F1" ) + v[1] );
	v[0] = v[2] * max( 0, V( "_TC" ) - VL( "_Loans", 1 ) );// free cash to lend
}
else
	v[0] = -1;									// no limit
...
VS( GRANDPARENT, "SavForc" );					// ensure savings are calculated

// workers forced savings for basic goods plus planned savings for luxury goods
v[0] = V( "_fd" ) * ( VS( GRANDPARENT, "SavBas" ) + VS( GRANDPARENT, "SavLux" ) );
