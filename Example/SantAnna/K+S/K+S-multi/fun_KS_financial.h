...
switch ( ( int ) VS( PARENT, "flagFiscalRule" ) )		// fiscal rule to use
{
	...
	case 2:												// soft max deficit rule
		if ( VLS( PARENT, "dGDPreal", 1 ) < 0 )			// negative GDP growth?
		{
...
// Taylor rule
v[2] = V( "rT" ) + V( "gammaPi" ) * ( VLS( PARENT, "dCPIb", 1 ) - V( "piT" ) ) +
...
CYCLES( PARENT, cur, "Capital" )						// build ranking for s.1
	CYCLES( cur, cur1, "Firm1" )
	{
		v[1] = VS( cur1, "_NW1" );						// net worth
		v[2] = VS( cur1, "_S1" );						// sales
		firmData.NWtoS = ( v[2] > 0 ) ? v[1] / v[2] : v[1];// cash to sales ratio
		firmData.firm = cur1;
		rank1.push_back( firmData );
	}

CYCLES( PARENT, cur, "Consumption" )					// build ranking for s.2
	CYCLES( cur, cur2, "Firm2" )						// for each industry
	{
		v[1] = VS( cur2, "_NW2" );
		v[2] = VS( cur2, "_S2" );
		firmData.NWtoS = ( v[2] > 0 ) ? v[1] / v[2] : v[1];
		firmData.firm = cur2;
...
//V( "bankMaps" );										// ensure vector updated
