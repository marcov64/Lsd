...
origVar   <- c( "_A2", "_L2", "_Q2e", "_f2", "_post2chg", "_s2avg", "_w2realAvg" )
  ...
  for( i in 1 : nTsteps ) {                # all time steps
    ...
    for( j in 1 : nElemMC ) {             # and all element instances
      ...
      if( is.na( mc[ i, "Q2e", j ] ) || mc[ i, "Q2e", j ] < 1 ||
          ...
          is.na( mc[ i, "post2chg", j ] ) || is.na( mc[ i, "L2", j ] ) ) {
        mc[ i, , j ] <- NA
        next
      }
      ...
      if( mean.A2 != 0 ) {
        ...
        if( ! is.na( mc[ i, "post2chg", j ] ) ) {
          if( mc[ i, "post2chg", j ] == 0 )
...
warmUp    <- 100     # number of "warm-up" time steps
