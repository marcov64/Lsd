...
CYCLE( cur, "Firm1" )
{
	if ( quit[ i ] )
	{
		if ( h > 0 || i != k )					// firm must exit?
		{
			...
			CFUNS( cur, exit_firm, NULL );		// del obj & collect liq. value
		}
		else
...
while ( j - i > 0 && ita != appl->end( ) )
{
	...
	if ( ROUND( ita->w, v[2], 0.01 ) <= v[2] )
	{
		...
		CFUNS( ita->wrk, hire_worker, 1, THIS, v[2] );// set firm, vintage & wage
		++i;									// scaled count hire
	}
...
*/
RESULT( CFUN( mov_avg_bound, "A1", VS( PARENT, "mLim" ), VS( PARENT, "mPer" ) ) )
