...
double dDepo = Depo - VLS( FINSECL1, "Depo", 1 );// change in deposits
...
double PiB = VS( FINSECL1, "PiB" );				// profits
double r = VS( FINSECL1, "r" );					// interest rate
...
double Taxes = - TaxW - TaxDiv - Tax1 - Tax2 + Tax;
double Profits = - netPi1 + netPi1 - netPi2 + netPi2 - PiB + PiB;
double Dividends = + Div_1 - Div1_1 - Div2_1;
...
v[3] = abs( Consumption ) + abs( Investment ) + abs( GovExpend ) + abs( Wages ) +
	   ...
	   abs( LiqEquity ) + abs( BadDebt ) + abs( DepoIntrst ) + abs( LoanIntrst ) +
	   abs( DepoChg ) + abs( LoanChg ) + abs( GovDebtChg ) + abs( GovDepoChg );
...
double firms2nl = - Inom + netPi2 - Div2_1 + cEntry2_1 - cExit2_1
				  + BadDeb2_1;
double bankNL = - PiB - BadDeb_1 - iDb + iB;
double govtNL = - G + Tax + PiB;

v[4] = workersNL + firms1nl + firms2nl + bankNL + govtNL;
...
double nonNeg[ ] = { CD, CDc, CS, Deb1, Deb2, Depo, DepoG, Loans, BadDeb,
					 BadDeb1, BadDeb2, Bda, SavAcc, NW1, NW2, iB, iDb };
double posit[ ] = { };
...
CFUN( check_error, abs( - PiB - BadDeb_1 - iDb + iB
				  		- ( ( Loans - Depo ) - ( Loans_1 - Depo_1 ) )
				  		- ( Deb - Deb_1 ) + ( DepoG - DepoG_1 ) ) > TOL,
...
CFUN( check_error, abs( - C + G + W - TaxW + Div_1 - cEntry_1 + cExit_1
				  		+ r * SavAcc_1 - ( SavAcc - SavAcc_1 ) ) > TOL,
	  "INCONSISTENT-SFC-FLOW", 0, & errors );
...
CYCLES( CAPSECL1, cur, "Firm1" )
{
	...
	double nonNeg[ ] = { _CS1, _CD1, _CD1c, _Div1, _Eq1, _HC, _NC, _RD, _D1, _Q1,
						 ...
						 _S1, _Tax1, _W1, _f1, _i1, _iD1, _imi, _inn };
...
CYCLES( CONSECL1, cur, "Firm2" )
{
	...
	double nonNeg[ ] = { _A2, _CD2, _CD2c, _CI, _CS2, _D2, _D2d, _D2e, _Deb2,
						 ...
						 _f2, _i2, _iD2, _l2, _life2cycle, __pVint, _t2ent };
