...
for( k in 1 : nExp ) {
  table[ 1, k ] <- nSize
  table[ 2, k ] <- mean( Adata[[ k ]]$dGDP[ TmaskStat ], na.rm = TRUE )
  table[ 3, k ] <- mean( Adata[[ k ]]$U[ TmaskStat ], na.rm = TRUE )
  table[ 4, k ] <- mean( Adata[[ k ]]$dEm[ TmaskStat ], na.rm = TRUE )
  table[ 5, k ] <- mean( Adata[[ k ]]$shockAavg[ TmaskStat ], na.rm = TRUE )
  table[ 6, k ] <- log( Adata[[ k ]]$Em[ nTstat ] )
...
subTitle <- paste( "( MC runs =", nSize, "/ period =", warmUpStat + 1, "-",
                   nTstat, ")" )
