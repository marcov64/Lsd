...
double pE = VLS( ENESECL2, "pE", 1 );			// energy price
...
*/
RESULT( VL( "_Deb1", 1 ) * VS( FINSECL2, "rDeb" ) * 
...
RESULT( V( "_W1" ) +							// labor effective total cost
		V( "_En1" ) * VLS( ENESECL2, "pE", 1 ) +// energy total cost
...
v[1] = V( "_S1" ) - V( "_C1" );					// gross operating margin
v[2] = VS( FINSECL2, "rD" ) * VL( "_NW1", 1 );	// financial income
v[3] = V( "_i1" ); 								// financial expense (interest)
...
RESULT( ( VS( LABSUPL2, "w" ) / V( "_BtauLP" ) +
		  ( VLS( ENESECL2, "pE", 1 ) + VS( GRANDPARENT, "trCO2" ) * V( "_BtauEF" ) ) / 
...
RESULT( VS( LABSUPL2, "w" ) / V( "_AtauLP" ) + 
	    ( VLS( ENESECL2, "pE", 1 ) + VS( GRANDPARENT, "trCO2" ) * 
		V( "_AtauEF" ) ) / V( "_AtauEE" ) )
