...
if sim.options( zip = False, csv = True ) :	# apply non-default configurations
	...
	if sim.config( "l0=10_l0radius=25_T=500.lsd" ) :	# load existing configuration
		...
		for t in range( 1, sim.lastT + 1 ) :	# run time steps one at a time
			...
			print( "g=%.2f | e=%g | m=%g" % ( sim.root.searchElem( "g" ).values[ 0 ], sim.root.searchElem( "e" ).values[ 0 ], sim.root.searchElem( "m" ).values[ 0 ] ) )
...
print( "g=", sim.root.searchElem( "g" ).data )
print( "e=", sim.root.searchElem( "e" ).data )
print( "m=", sim.root.searchElem( "m" ).data )
