...
CFUN( check_error, L1 != v[7] * Lscale || L1 != v[10] * Lscale,
	  "INCONSISTENT-LABOR-S1", 0, & errors );

CFUN( check_error, L2 < v[8] * Lscale || v[8] != v[11] || v[12] != v[11] * Lscale,
	  "INCONSISTENT-LABOR-S2", 0, & errors );

CFUN( check_error, Us > Ue || Ue > U,
	  "INCONSISTENT-UNEMPLOYMENT", 0, & errors );

CFUN( check_error, wzErr.size( ) > 0,
	  "ZERO-WAGE-WORKERS", wzErr.size( ), & errors );
...
CFUN( check_error, W1 + W2 + Bon2 > ( 1 + TOL ) * ( v[6] + v[9] ) ||
	  round( W ) != round( W1 + W2 ) || round( Bon ) != round( Bon2 ),
	  "INCONSISTENT-PAYROLL", 0, & errors );

CFUN( check_error, wAvg + Bon / L < InAvg || wrAvg < wsAvg || W <= TaxW,
...
CFUN( check_error, abs( - C + G + W - TaxW + Bon_1 + Div_1 - cEntry_1 + cExit_1
...
CFUN( check_error, round( W1 ) != round( v[7] ),
	  "INCONSISTENT-PAYROLL", 0, & errors );
...
CFUN( check_error, abs( - Inom + ( Pi2 - Tax2 ) - Bon2_1 - Div2_1 + cEntry2_1
...
CYCLES( CAPSECL1, cur, "Firm1" )
{
	...
	CFUN( check_error, abs( _S1 - _W1 - _Tax1 - ( _Pi1 - _Tax1 ) + _iD1 - _i1 ) > TOL,
		  "INCONSISTENT-SFC-FLOW", 0, & errors );
...
CYCLES( CONSECL1, cur, "Firm2" )
{
	...
	CFUN( check_error, noWrkVintErr.size( ) > 0,
		  "INCONSISTENT-VINT-PROD", noWrkVintErr.size( ), & errors );
	...
	CFUN( check_error, _w2o > _w2o_1 * wCap || _w2o < _w2o_1 / wCap,
	...
	CFUN( check_error, abs( _S2 - _W2 - _Tax2 - ( _Pi2 - _Tax2 ) + _iD2 - _i2 ) > TOL,
		  "INCONSISTENT-SFC-FLOW", 0, & errors );
