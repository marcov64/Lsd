...
VS( PARENT, "_suppl2" );						// ensure supplier is selected
cur = PARENTS( SHOOKS( HOOKS( PARENT, SUPPL ) ) );// pointer to supplier

// single-stage cost advantage of one new machine
v[1] = VLS( PARENT, "_w2avg", 1 ) * ( 1 / V( "_Avint" ) - 1 / VS( cur, "_A1" ) );

// if new machine cost is not better in absolute terms or
// payback period of replacing one machine in current vintage is over b
if ( v[1] <= 0 || VS( cur, "_p1" ) / v[1] > VS( GRANDPARENT, "b" ) )
...
v[9] = VS( GRANDPARENT, "m2" ) / VS( GRANDPARENT, "k2" );// K final productivity

// vintage notional production considering the production stages (complexity)
v[1] = v[9] * V( "_Avint" ) * VS( LABSUPL3, "Lscale" );

// required production capacity considering the production stages
v[2] = V( "_toUseVint" ) * v[9];
...
RESULT( v[2] * ( VS( GRANDPARENT, "m2" ) * v[1] / VS( GRANDPARENT, "k2" ) ) *
		VS( LABSUPL3, "Lscale" ) )
...
/*
Effective notional (single-stage) labor productivity of vintage
