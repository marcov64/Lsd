/******************************************************************************

	FIRM1 OBJECT EQUATIONS
	----------------------

	Written by Marcelo C. Pereira, University of Campinas

	Copyright Marcelo C. Pereira
	Distributed under the GNU General Public License

	Equations that are specific to the Firm1 objects in the K+S LSD model
	are coded below.

 ******************************************************************************/

/*============================== KEY EQUATIONS ===============================*/

EQUATION( "_A1" )
/*
Labor productivity (single-stage) of new vintage of machine
when employed for production
Also updates '_B1'
*/

double _Ainn, _Binn, _Aimi, _Bimi;

double xi = VS( PARENT, "xi" );					// share of R&D for innovation
double _A1 = CURRENT;							// previous period productivities
double _B1 = VL( "_B1", 1 );
int g1front = VS( PARENT, "g1front" );			// tech. frontier generation
int _g1 = h = VL( "_g1", 1 );					// previous period generation

_Ainn = _Binn = _Aimi = _Bimi = 0;				// assume innov./imit. failure

// normalized workers on R&D of the firm, handling entrants external R&D people
double _L1rd = V( "_t1ent" ) > T - 1 ? VL( "_L1rd", 1 ) :
									   VL( "_RD1", 1 ) / VLS( PARENT, "w1avg", 1 );
double _L1rdN = _L1rd * VS( LABSUPL2, "Ls0" ) / VLS( LABSUPL2, "Ls", 1 );

if ( _g1 < g1front )							// radical innovation possible?
{
	// radical (product) innovation process (success probability)
	v[1] = 1 - exp( - VS( PARENT, "zeta0" ) * xi * _L1rdN );

	if ( bernoulli( v[1] ) )					// radical innovation succeeded?
	{	// next tech. gen. object
		cur = V_EXTS( GRANDPARENT, countryE, g1ptr[ _g1 + 1 ] );
		_Ainn = VS( cur, "_A1g" );				// new tech. gen. productivities
		_Binn = VS( cur, "_B1g" );
		h = VS( cur, "_IDg" );
	}
}

if ( h == _g1 )									// no radical innovation?
{
	// incremental (process) innovation process (success probability)
	v[1] = 1 - exp( - VS( PARENT, "zeta1" ) * xi * _L1rdN );

	if ( bernoulli( v[1] ) )					// incremental inno. succeeded?
	{
		double alpha1 = VS( PARENT, "alpha1" );	// beta distrib. alpha parameter
		double beta1 = VS( PARENT, "beta1" );	// beta distrib. beta parameter
		double x1inf = VS( PARENT, "x1inf" );	// lower beta inno. draw support
		double x1sup = VS( PARENT, "x1sup" );	// upper beta inno. draw support

		// new final productivity (A) from innovation
		_Ainn = _A1 * ( 1 + x1inf + beta( alpha1, beta1 ) * ( x1sup - x1inf ) );

		// new production productivity (B) from innovation
		_Binn = _B1 * ( 1 + x1inf + beta( alpha1, beta1 ) * ( x1sup - x1inf ) );
	}
}

// imitation process (success probability)
v[2] = 1 - exp( - VS( PARENT, "zeta2" ) * ( 1 - xi ) * _L1rdN );

if ( bernoulli( v[2] ) )						// imitation succeeded?
{
	k = VS( PARENT, "F1" );						// number of firms in sector 1
	dblVecT imiProb( k );						// vector for tech distance

	v[3] = i = 0;								// inverse distance/firm accum.
	CYCLES( PARENT, cur1, "Firm1" )				// 1st run: abs. inv. distance
		if ( cur1 == THIS )
			imiProb[ i++ ] = 0;					// can't self-imitate
		else
		{
			v[4] = sqrt( pow( VLS( cur1, "_A1", 1 ) - _A1, 2 ) +
						 pow( VLS( cur1, "_B1", 1 ) - _B1, 2 ) );
			v[3] += imiProb[ i++ ] = ( v[4] > 0 ) ? 1 / v[4] : 0;
		}

	if ( v[3] > 0 )
	{
		v[5] = i = 0;							// probabilities/firm accum.
		CYCLES( PARENT, cur1, "Firm1" )			// 2nd run: cumulative imi. prob.
		{
			v[5] += imiProb[ i ] / v[3];		// normalize to add up to 1
			imiProb[ i++ ] = v[5];
		}

		// draw a firm to imitate according to the distance probabilities
		j = upper_bound( imiProb.begin( ), imiProb.end( ), RND ) - imiProb.begin( );

		if ( j < k )							// at least one firm reachable?
		{
			cur2 = TSEARCHS( PARENT, "Firm1", j + 1 );// get pointer to firm

			_Aimi = VLS( cur2, "_A1", 1 );		// get imitated firm productivities
			_Bimi = VLS( cur2, "_B1", 1 );
		}
	}
}

// select best option between the three options (current/innovation/imitation)
v[6] = v[7] = v[8] = 0;

if ( _Ainn * _Binn > _A1 * _B1 )				// is innovation better?
{
	_A1 = _Ainn;								// new _A1
	_B1 = _Binn;								// new _B1
	v[6] = ( h > _g1 ) ? 1 : 0;					// change to new generation?
	v[7] = ( h > _g1 ) ? 0 : 1;
	v[8] = 0;									// no imitation
}
else
	h = _g1;									// keep generation

if ( _Aimi * _Bimi > _A1 * _B1 )				// is imitation better (yet)?
{
	_A1 = _Aimi;
	_B1 = _Bimi;
	v[6] = v[7] = 0;							// no innovation
	v[8] = 1;									// imitation succeeded
	h = VLS( cur2, "_g1", 1 );					// imitated generation
}

WRITE( "_B1", _B1 );
WRITE( "_g1", h );
WRITE( "_inn1r", v[6] );
WRITE( "_inn1i", v[7] );
WRITE( "_imi1", v[8] );

// add entry to the map of vintage productivity and skill
i = V( "_ID1" );								// firm ID
j = VNT( T, i );								// vintage ID
k = VS( GRANDPARENT, "flagWorkerLBU" );			// learning mode
if ( k == 0 || k == 2 )							// no learning by using
	v[9] = INISKILL;							// maximum skills
else
	v[9] = VS( PARENT, "sigma" );

WRITE_EXTS( GRANDPARENT, countryE, vintProd[ j ].sVp, v[9] );
WRITE_EXTS( GRANDPARENT, countryE, vintProd[ j ].sVavg, v[9] );

RESULT( _A1 )


EQUATION( "_Deb1max" )
/*
Prudential maximum bank debt of firm in capital-good sector
*/

// maximum debt allowed to firm, considering net worth and operating margin
v[5] = VS( FINSECL2, "Lambda" ) * max( VL( "_NW1", 1 ),
									   VL( "_S1", 1 ) - VL( "_W1", 1 ) );

// apply an absolute floor to maximum debt prudential limit
v[0] = max( v[5], VS( FINSECL2, "Lambda0" ) * VLS( GRANDPARENT, "PPI", 1 ) );

WRITE( "_cred1c", 0 );							// reset constraint for period

RESULT( v[0] )


EQUATION( "_NC1" )
/*
Number of new client firms in the period.
Also creates the client-supplier connecting objects.
*/

firmMapT firms = V_EXTS( GRANDPARENT, countryE, firm2map );// list with all firms
h = firms.size( );								// number of firms in sector 2
k = V( "_HC1" );								// number of existing clients
v[1] = VS( PARENT, "g1max" );					// machine tech. top generation

CYCLE( cur, "Cli" )								// remove current clients
	firms.erase( ( int ) VS( cur, "_IDc" ) );

// if has top tech, go after new luxury industry firms of same base technology
v[0] = 0;
if ( v[1] > 0 && V( "_g1" ) == v[1] )
	for ( firmMapT::iterator it = firms.begin( ); it != firms.end( ); ++it )
		if ( VS( PARENTS( it->second ), "g2base" ) == v[1] )
		{
			CFUN( send_brochure, it->second );
			++k;
			++v[0];
		}

i = ceil( VS( PARENT, "gamma" ) * k );			// new clients in period
i = min( max( i, 1 ), min( h - k, firms.size( ) ) );// in [1, F2 - HC]

v[2] = max( 1, ceil( h / VS( PARENT, "F1" ) ) );// firm fair share

if ( k + i < v[2] )								// ensure at least fair share
	i = v[2] - k;

// build vector of all target firms (not yet clients)
vector < firmPairT > targets( firms.begin( ), firms.end( ) );

// draw new clients from target list, updating the list after each draw
for ( ; i > 0 && targets.size( ) > 0; --i, ++v[0] )
{
	h = uniform_int( 0, targets.size( ) - 1 );	// draw a index in the list
	firmPairT client = targets[ h ];			// retrieve drawn map pair
	targets.erase( targets.begin( ) + h );		// remove drawn firm from list

	// create the brochure/client interconnected objects
	CFUN( send_brochure, client.second );
}

RESULT( v[0] )


EQUATION( "_Q1" )
/*
Planed production for a firm in capital-good sector
*/

v[1] = V( "_p1" );								// machine price
v[2] = V( "_cred1" );							// available credit
v[3] = VL( "_NW1", 1 );							// net worth (cash available)
v[4] = V( "_c1" );								// unit cost
v[5] = V( "_D1" ) / v[1];						// potential production (# mach)
v[6] = V( "_RD1" );								// R&D costs still to pay

v[7] = v[5] * ( v[4] - v[1] ) + v[6];			// cash flow to fulfill orders

if ( v[7] < 0 || v[7] <= v[3] - 1 )				// firm can self-finance?
{
	v[0] = v[5];								// plan the desired output
	v[3] -= v[7];								// remove flow from cash
}
else
{
	if ( v[7] <= v[3] - 1 + v[2] )				// possible to finance all?
	{
		v[0] = v[5];							// plan the desired output
		v[8] = v[9] = v[7] - v[3] + 1;			// finance the difference
		v[3] = 1;								// keep minimum cash
	}
	else										// credit constrained firm
	{
		// produce as much as the available finance allows
		v[0] = ( v[3] - 1 - v[7] + v[2] ) / v[4];// max possible
		v[0] = min( max( v[0], 0 ), v[5] );		// positive but up to D1

		v[8] = v[2];							// finance what is possible
		v[9] = v[6] - v[3] + 1;					// desired credit

		if ( v[0] == 0 )
		{
			v[10] = 1;							// all orders canceled
			v[3] -= v[6] - v[2];				// let negative NW (bankruptcy)
		}
		else
		{
			v[10] = 1 - v[0] / v[5];			// machine shortage factor
			v[3] = 1;							// keep minimum cash
		}

		// shrink or cancel all exceeding orders
		CYCLE( cur, "Cli" )
			if ( VS( cur, "_tOrd" ) == T )		// order in this period?
			{
				if ( v[10] == 1 )				// bankruptcy?
					INCRS( cur, "_nCan", VS( cur, "_nOrd" ) );
				else
					INCRS( cur, "_nCan", VS( cur, "_nOrd" ) * v[10] );
			}
	}

	CFUN( update_debt1, v[9], v[8] );			// update firm debt
}

// provision for revenues and expenses
WRITE( "_NW1", v[3] );							// update the firm net worth

RESULT( v[0] )


EQUATION( "_RD1" )
/*
R&D expenditure of firm in capital-good sector
*/

v[1] = VL( "_S1", 1 );							// sales in previous period
v[2] = VS( PARENT, "nu" );						// R&D share of sales

if ( v[1] > 0 )
	v[0] = v[2] * v[1];
else											// no sales
	// keep current expenditure or a share of available cash
	v[0] = min( CURRENT, v[2] * VL( "_NW1", 1 ) );

// always hire at least one worker
RESULT( max( v[0], VLS( PARENT, "w1avg", 1 ) ) )


EQUATION( "_Tax1" )
/*
Total tax paid by firm in capital-good sector
Also updates final net wealth on period
*/

v[1] = V( "_Pi1" );								// firm profit in period
v[2] = VS( GRANDPARENT, "tr" );					// tax rate

if ( v[1] > 0 )									// profits?
{
	v[0] = v[1] * v[2];							// tax to government
	v[4] = VS( PARENT, "d1" ) * ( v[1] - v[0] );// dividend to shareholders
}
else
	v[0] = v[4] = 0;							// no tax/dividend on losses

WRITE( "_Div1", v[4] );							// save period dividends

// compute free cash flow
v[6] = v[1] - v[0] - v[4];

// remove from net wealth the provision for revenues and expenses
v[7] = INCR( "_NW1", V( "_Q1" ) * ( V( "_c1" ) - V( "_p1" ) ) + V( "_RD1" ) );

if ( v[6] < 0 )									// must finance losses?
{
	if ( v[7] >= - v[6] + 1 )					// can cover losses with reserves?
		INCR( "_NW1", v[6] );					// draw from net wealth
	else
	{
		v[8] = V( "_cred1" );					// available credit
		v[9] = - v[6] - v[7] + 1;				// desired finance

		if ( v[8] >= v[9] )						// can finance losses?
		{
			CFUN( update_debt1, v[9], v[9] );	// finance all
			WRITE( "_NW1", 1 );					// minimum net wealth
		}
		else
		{
			CFUN( update_debt1, v[8], v[8] );	// take what is possible
			INCR( "_NW1", v[6] - v[8] );		// let negative NW (bankruptcy exit)
		}
	}
}
else											// pay debt with available cash
{
	v[10] = V( "_Deb1" );						// current debt

	if ( v[10] > 0 )							// has debt?
	{
		if ( v[6] > v[10] )						// can repay all debt and more
		{
			CFUN( update_debt1, 0, - v[10] );	// zero debt
			INCR( "_NW1", v[6] - v[10] );		// save the rest
		}
		else
			CFUN( update_debt1, 0, - v[6] );	// repay part of debt
	}
	else
		INCR( "_NW1", v[6] );					// save all
}

RESULT( v[0] )


EQUATION( "_p1" )
/*
Price of good of firm in capital-good sector
*/
RESULT( ( 1 + VS( PARENT, "mu1" ) ) * V( "_c1" ) )


/*============================ SUPPORT EQUATIONS =============================*/

EQUATION( "_BC1" )
/*
Number of buying clients for firm in capital-good sector
*/
RESULT( COUNT_CND( "Cli", "_tOrd", "==", T ) )


EQUATION( "_D1" )
/*
Potential demand (in money terms) received by a firm in
capital-good sector
*/
VS( GRANDPARENT, "Id" );						// make sure all orders are sent
RESULT( SUM_CND( "_nOrd", "_tOrd", "==", T ) * V( "_p1" ) )


EQUATION( "_HC1" )
/*
Number of historical client firms from consumer-good sector.
Also removes old, non-buying clients.
*/

i = 0;											// client counter
CYCLE_SAFE( cur, "Cli" )						// remove old clients
{
	if ( VS( cur, "_tSel" ) < T - 1 )			// last selection is old?
	{
		DELETE( SHOOKS( cur ) );				// remove supplier brochure entry
		DELETE( cur );							// remove client entry
	}
	else
		++i;
}

RESULT( i )


EQUATION( "_L1" )
/*
Labor employed by firm in capital-good sector
Includes R&D labor
*/

v[1] = VS( PARENT, "L1" );						// total labor in sector 1
v[2] = VS( PARENT, "L1d" );						// total labor demand in sector 1
v[3] = VS( PARENT, "L1rd" );					// R&D labor in sector 1
v[4] = VS( PARENT, "L1dRD" );					// R&D labor demand in sector 1
v[5] = V( "_L1d" );								// firm total labor demand
v[6] = V( "_L1rd" );							// firm R&D labor
v[7] = V( "_L1dRD" );							// firm R&D labor demand

// total workers in firm after possible shortage of workers
RESULT( min( v[6] + ( v[2] > v[4] ? round( ( v[5] - v[7] ) * ( v[1] - v[3] ) /
										   ( v[2] - v[4] ) ) : 0 ), v[5] ) )


EQUATION( "_L1d" )
/*
Labor demand of firm in capital-good sector
Includes R&D labor
*/
RESULT( V( "_L1dRD" ) + ceil( V( "_Q1" ) / ( VS( PARENT, "m1" ) * V( "_B1" ) ) ) )


EQUATION( "_L1dRD" )
/*
R&D labor demand of firm in capital-good sector
*/
RESULT( ceil( V( "_RD1" ) / VLS( PARENT, "w1avg", 1 ) ) )


EQUATION( "_L1rd" )
/*
R&D labor employed by firm in capital-good sector
*/
v[1] = VS( PARENT, "L1dRD" );
RESULT( v[1] > 0 ? ceil( V( "_L1dRD" ) * VS( PARENT, "L1rd" ) / v[1] ) : 0 )


EQUATION( "_Pi1" )
/*
Profit of firm in capital-good sector
*/
RESULT( V( "_S1" ) - V( "_W1" ) - V( "_i1net" ) )// firm profits before taxes


EQUATION( "_Q1e" )
/*
Effective output of firm in capital-good sector
*/

v[0] = V( "_Q1" );								// planned production
v[1] = V( "_L1" );								// effective labor available
v[2] = V( "_L1d" );								// desired total workers

// required labor available?
if ( v[1] >= v[2] || VS( GRANDPARENT, "flagMachDeliv" ) == 0 )
	END_EQUATION( v[0] );						// produce as planned

v[3] = V( "_L1rd" );							// effective R&D workers
v[4] = V( "_L1dRD" );							// desired R&D workers

// adjustment factor
v[5] = v[2] > v[4] ? 1 - ( v[1] - v[3] ) / ( v[2] - v[4] ) : 1;

// adjust all exceeding orders, supplying at least one machine
CYCLE( cur, "Cli" )
	if ( VS( cur, "_tOrd" ) == T )				// order in this period?
	{
		v[6] = VS( cur, "_nOrd" ) - VS( cur, "_nCan" );// existing net orders
		v[0] -= v[7] = min( v[6] * v[5], v[6] - 1 );
		INCRS( cur, "_nCan", v[7] );
	}

RESULT( max( v[0], 0 ) )						// avoid negative in part. cases


EQUATION( "_S1" )
/*
Sales (in money terms) of firm in capital-good sector
*/
RESULT( V( "_p1" ) * V( "_Q1e" ) )


EQUATION( "_W1" )
/*
Total wages paid by firm in capital-good sector
*/
RESULT( V( "_L1" ) * VS( PARENT, "w1avg" ) )


EQUATION( "_c1" )
/*
Unit cost of production (1 machine) of firm in capital-good sector.
Use sectoral average (pool of sector 1 workers).
*/
V( "_A1" );									// ensure innovation process ok
RESULT( VLS( PARENT, "w1avg", 1 ) / ( VS( PARENT, "m1" ) * V( "_B1" ) ) )


EQUATION( "_f1" )
/*
Market share of firm in capital-good sector
Keep market share if sector didn't produce
*/
v[1] = VS( PARENT, "Q1e" );
RESULT( v[1] > 0 ? V( "_Q1e" ) / v[1] : CURRENT )


EQUATION( "_i1net" )
/*
Net interest expenses (income) by firm in capital-good industry
*/

v[1] = VLS( FINSECL2, "rDeb", 1 ) * ( 1 + ( VL( "_qc1", 1 ) - 1 ) *
	   VS( FINSECL2, "kConst" ) );  			// effective debt interest rate
v[0] = v[1] * VL( "_Deb1", 1 );					// interest to pay
v[0] -= VS( FINSECL2, "rD" ) * VL( "_NW1", 1 );	// financial income

RESULT( v[0] )


/*========================== SUPPORT LSD FUNCTIONS ===========================*/

EQUATION( "_cred1" )
/*
Bank credit available (new debt) to firm in capital-good sector
Function called multiple times in single time step
*/

v[1] = V( "_Deb1" );							// current firm debt
v[2] = V( "_Deb1max" );							// maximum prudential credit

if ( v[2] > v[1] )								// more credit possible?
{
	v[0] = v[2] - v[1];							// potential free credit

	cur = HOOK( BANK );							// firm's bank
	v[3] = VS( cur, "_TC1free" );				// bank's available credit

	if ( v[3] > -0.1 )							// credit limit active
		v[0] = min( v[0], v[3] );				// take just what is possible
}
else
	v[0] = 0;									// no credit available

RESULT( v[0] )


/*============================= DUMMY EQUATIONS ==============================*/

EQUATION_DUMMY( "_B1", "_A1" )
/*
Labor productivity when producing the new vintage of machines
Updated in '_A1'
*/

EQUATION_DUMMY( "_Deb1", "" )
/*
Stock of bank debt of firm in capital-good sector
Updated in '_Q1', '_Tax1'
*/

EQUATION_DUMMY( "_Div1", "_Tax1" )
/*
Dividends paid by firm in capital-good sector
Updated in '_Tax1'
*/

EQUATION_DUMMY( "_NW1", "" )
/*
Net wealth (free cash) of firm in capital-good sector
Updated in '_Q1' and '_Tax1'
*/

EQUATION_DUMMY( "_cred1c", "" )
/*
Credit constraint for firm in capital-good sector
Updated in '_Deb1max', '_Q1'
*/

EQUATION_DUMMY( "_g1", "_A1" )
/*
Current machine generation in production by capital-good firm
Updated in '_A1'
*/

EQUATION_DUMMY( "_imi1", "_A1" )
/*
Imitation success (1) or failure (0) for firm in capital-good sector
Updated in '_A1'
*/

EQUATION_DUMMY( "_inn1i", "_A1" )
/*
Incremental nnovation success (1) or failure (0) for firm in capital-good sector
Updated in '_A1'
*/

EQUATION_DUMMY( "_inn1r", "_A1" )
/*
Radical innovation success (1) or failure (0) for firm in capital-good sector
Updated in '_A1'
*/

EQUATION_DUMMY( "_qc1", "" )
/*
Credit class of firm in sector 1 (1,2,3,4)
Updated in 'cScores'
*/
