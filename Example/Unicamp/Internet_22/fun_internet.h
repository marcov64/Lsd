...
#define _VS(X,Y) X->cal(NULL,(char*)Y,0,false)		// for simpler handling
#define _VLS(X,Y,Z) X->cal(NULL,(char*)Y,Z,false)
...
void rank_prov_global::update( object *firstProv, unsigned int window )
{
	...
	for ( cur = firstProv; cur != NULL; cur = cur->next )
	{													// scan all providers
...
rank_prov_user::rank_prov_user( object *User ) : User( User )
{
	...
	for ( curl = User->node->first; curl != NULL; curl = curl->next )
	{												// scan all neighbors
		nProvPtr = curl->to->hook;					// pointer to neighbor provider
...
// Choose separator
#define fsep( dir ) ( dir[0] == '\0' ? "" : "/" )

long equation::init_net( object *container, char const *lab, char const *dir, char const *base_name )
{
	int typeNet;
