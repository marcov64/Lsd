...
#define SAVE_NETW	false				// flag for saving network to file (end)
#define BASE_NAME 	(char*) "network"	// input and output network base filename
#define IN_DIR 		(char*) "facebook"	// input network file folder path
#define NUM_NETS 	10					// number of random network realizations


#define STD_HHI		false				// use standardized/normalized HHI
...
// C++ support functions declaration
#define EQ_USER_CFUNS long init_net( object *container, char const *lab, \
							 		 char const *dir, char const *base_name );

#include "fun_internet.h"				// C++ support functions definition

struct rank_strat
{
	double rsinc[RANK_DIM][STRAT_INC+1],
		   rsentr[RANK_DIM][STRAT_ENTR+1];	// global strategies ranking matrixes
};

// global strategy rank matrix, provider rank, pointer to users container
rank_strat *r;
rank_prov_global *rp;
object *usernet;
...
*/
	r=new rank_strat;					// allocate a new rank matrix object
	...
	NO_POINTER_CHECK;					// allow saving "invalid" object address
	WRITE_SHOOK(reinterpret_cast<object *>(r));// pointer to rank matrix object
	USE_POINTER_CHECK;

	rp=new rank_prov_global;			// allocate a provider rank object
	...
	v[11]=round(V("Nprovinit"));		// # of initial providers
	...
	v[16]=round(V("pop0"));				// # of initial users
	v[17]=V("Baver");					// initial average budget
	v[18]=V("Bvar");					// initial budget standard deviation
	v[42]=V("Bmin");					// minimum budget
	v[19]=V("b1min");					// min user price sensitivity
	v[20]=V("b1b2");					// user price/(price+quality) sensitivity share
	v[33]=V("b3min");					// min user social network sensitivity
	v[34]=V("b3max");					// max user social network sensitivity
	...
	v[45]=V("fixstrat");				// define fixed strategies for providers
	...
	WRITE_SHOOKS(cur6,SEARCHS(cur6,"Technology"));	// saves pointer to top technology
	...
	CYCLE(cur1,"Provider")				// initializes every provider
	{
		...
		WRITES(cur1,"g",0);				// no initial revenue growth
		...
		if(!v[45])						// if not using fixed strategies
		...
		WRITE_SHOOKS(cur2,cur5);		// saves pointer to technology
	}
	...
	SHUFFLE_NETS(cur3,"User");			// shuffle the net node order

	v[2]=1;								// user counter
	v[47]=v[48]=v[49]=0;				// b parameter accumulators
	CYCLE(cur4,"User")					// initialize each user
	{
		if(v[2]<=v[16])					// if one of the initial users
		{
			WRITES(cur4,"effUser",1);	// mark as internet effective user
			v[38]=uniform_int(1,v[11]);	// choose a provider
			WRITES(cur4,"prov",v[38]);
			WRITE_SHOOKS(cur4,SEARCH_CNDS(cur,"provID",v[38]));	// update pointer to provider
		...
		{
			WRITES(cur4,"effUser",0);	// mark as internet non-user
			WRITES(cur4,"prov",0);		// no initial provider
			WRITE_SHOOKS(cur4,NULL);
			v[43]=norm(v[17],v[18]);	// draw normal budget
			v[43]=max(v[43],v[42]);		// search until over minimum
			WRITES(cur4,"B",v[43]);		// initial budget
		}

		v[49]+=v[44]=uniform(v[33],v[34]);		// b3 draw
		v[36]=uniform(v[19],(1-v[44])*v[20]);	// b1 draw
		v[37]=uniform(0,(1-v[44])*(1-v[20]));	// b2 draw
		v[46]=(1-v[44])/(v[36]+v[37]);			// rescale b1 and b2
		v[47]+=v[36]*=v[46];					// so b1+b2+b3=1
		v[48]+=v[37]*=v[46];

		WRITES(cur4,"b1",v[36]);		// user price sensitivity
		WRITES(cur4,"b2",v[37]);		// user quality sensitivity
		WRITES(cur4,"b3",v[44]);		// user social network sensitivity
	...
	v[2]--;
	PLOG("\n Average b1 = %.2lf, b2 = %.2lf, b3 = %.2lf",
		 v[47]/v[2],v[48]/v[2],v[49]/v[2]);
...
EQUATION("pop")
/*
Consumer population total (users and wanna be users)
Only "enabled" users are counted
*/
	v[1]=V("gusers");					// customer growth rate
	v[2]=round(V("pop0"));				// initial customer population
	v[3]=round(V("popmax"));			// customer stable population
	v[4]=CURRENT;						// previous number of users

	v[0]=ceil(v[3]*v[2]*exp(v[1]*T)/(v[3]+v[2]*(exp(v[1]*T)-1)));
										// logistic curve growth users
	v[5]=0;								// total user counter
	CYCLE(cur,"User")					// do for all nodes (individuals)
	{
		v[5]++;							// one more existing potential user
		if(VS(cur,"effUser")==1)		// skip existing the first non-user
			continue;

		if(v[5]>v[0])					// if all new users already enabled
			break;						// stop

		WRITES(cur,"effUser",1);		// enable node as potential user
	}
...
EQUATION("B")
/*
Updade user budget for period (enable or disabled users)
*/
	v[0]=CURRENT;						// get last period budget
	v[1]=V("gB");						// and demand periodic growth
	v[2]=V("Bmin");						// minimum budget

	if(v[1]!=0 && v[0]>v[2])			// don't go below floor
	{
		v[3]=V("Bvar");					// initial budget variance

		v[0]=norm((1+v[1])*v[0],v[3]);	// new budget
	}
...
EQUATION("churn")
/*
Indicate the ocurrence of the churning of the user in the period
*/

v[1]=V("prov");							// get current period provider
v[2]=VL("prov",1);						// and past period's

if(v[2]!=0 && v[1]!=v[2])				// if had a provider before
										// and now have a different one
	v[0]=1;								// then there's churn
else
	v[0]=0;								// or not
...
EQUATION("provcov")
/*
Number of providers directly covered by seaching users, on top of the ones included
in the social network of the user
Defines a ranking for the available providers, mixing the best providers in three
sub-rankings: price, quality and price/quality
*/
	v[1]=V("Nprov");					// current number of providers
	v[2]=V("rankwindow");				// window size of sub-rankings

	rp->update(SEARCH("Provider"),(unsigned int)v[2]);	// update the global rank object

	v[3]=(double)rp->size();			// number of firms in main rank
	v[0]=v[3]/v[1];						// compute effective coverage
...
*/
	...
	if(!V("effUser"))					// if user not enabled yet, quits
		END_EQUATION(0);				// without selecting a provider

 	v[1]=CURRENT;						// gets current provider
	v[2]=V("t0");						// last contract start period
	v[3]=V("T");						// current contract duration
	if(v[1]!=0 && (T-v[2]<v[3]))		// user has valid contract?
		END_EQUATION(v[1]);				// nothing to do - keep the same provider

	V("provcov");						// make sure provider rank is updated
	v[4]=V("B");						// current budget
	v[5]=VL("Pavg",1);					// average market price
	v[6]=V("b1");						// low price sensitivity
	v[12]=V("b2");						// quality sensitivity
	v[7]=V("b3");						// social network sensitivity
	...
	v[26]=V("decmak");					// proportion of decision makers

	rank_prov_user rank=THIS;			// create the rank of providers to consider
	v[13]=rank.size();					// get the number of providers in the rank
	v[27]=rank.sNeighProv;				// fraction of neighbors with provider (eta)

	v[28]=min(v[27]/v[26],1);			// calculates eta
	v[29]=(1-v[28]*v[7])/(v[6]+v[12]);	// calculates rho
	v[31]=v[29]*v[6];					// b1~
	v[32]=v[29]*v[12];					// b2~
	v[33]=v[28]*v[7];					// b3~
	...
	if(v[1]==0)							// if no provider, try change
	{
		...
		cur=cur2=NULL;					// no firm object
	}
	else
		if((T-v[2])>=v[3])				// if contract expired, try change
		{
			v[14]=VLS(SHOOK,"M",1);		// gets last quality available
			v[15]=VS(SHOOK,"Pprov");	// current price for existing provider
			v[17]=rank.find_sk((unsigned long)v[1]);	// last share of existing provider

			if(v[15]<=v[4])				// check if current provider is affordable
			{
				v[16]=v[9]*pow(v[5]/v[15],v[31])*pow(v[14],v[32])*pow(0.5+v[17],v[33]);
	...
	for(v[25]=1, rank.pos(0); v[25] <= v[13]; v[25]++, rank.next())
	{									// scan all ranked providers
		cur1=rank.p();					// pointer to provider
		v[18]=rank.ID();				// gets provider ID
		v[20]=rank.M();					// last known (t-1) quality (precise)
		v[21]=rank.sk();				// last share of provider in neighborhood
		v[19]=VS(cur1,"Pprov");			// provider current (t) price

		if(v[18]!=v[1])					// it is not the current provider
		{
			v[22]=norm(v[20],v[8]*(1-v[28])*v[20]);	// so introduce error in quality measure
		...
		v[23]=pow(v[5]/v[19],v[31])*pow(v[22],v[32])*pow(0.5+v[21],v[33]);
...
*/
	...
	v[5]=VL("Nuser",4);
	v[6]=round(V("Tplan"));				// planning period
...
*/
	...
	cur=SEARCHS(GRANDPARENT,"NetEqTechnologies");	// Pointer to the technologies container
	cur=SHOOKS(cur);					// get top tech pointer
...
*/
	...
	v[7]=round(VS(GRANDPARENT,"Ttechdepr"));	// maximum technology lifespan
	...
	CYCLE(cur,"Network")				// scans all network technologies
	{
		...
		v[10]=VS(SHOOKS(cur),"cmtech");	// technology maintenance cost
		...
		v[13]=T-v[11];					// calculates network age
...
*/
	...
	CYCLES(GRANDPARENT,cur,"User")			// scans all users
	{
...
EQUATION("g")
/*
Provider's revenue growth (in log)
*/
	v[1]=V("R");							// current revenue
	v[2]=VL("R",1);							// previous revenue

	if(v[1]>0 && v[2]>0)
		v[0]=log(v[1])-log(v[2]);			// growth in log
	else
		if(v[1]<=0)							// no growth
			v[0]=0;
		else
			v[0]=999;						// conventional infinite growth
...
EQUATION("gavginc")
/*
Weighted average of all incumbent provider's revenue growth (in log)
*/
	v[1]=0;									// total revenue accumulator
	v[2]=0;									// t-1 revenue acummulator

	CYCLE(cur,"Provider")
	{
		v[3]=VS(cur,"group");				// get provider group
		if(v[3]!=1)							// if incumbent
		{
			v[1]+=VS(cur,"R");				// update current revenue
			v[2]+=VLS(cur,"R",1);			// update previous revenue
		}
	}
...
EQUATION("gavgentr")
/*
Weighted average of all entrant provider's revenue growth (in log)
*/
	v[1]=0;									// total revenue accumulator
	v[2]=0;									// t-1 revenue acummulator
...
*/
	...
	rank_strat *r=reinterpret_cast<rank_strat *>(SHOOKS(GRANDPARENT));
											// get pointer to rank matrix object
	v[1]=VL("strat",1);						// running strategy
	v[2]=V("laststratt0");					// last strategy change
	v[3]=round(V("nstratmin"));				// minimum time with strategy
	...
	v[17]=round(V("nexit"));				// maximum bad periods before exit
	v[18]=V("fixstrat");

	if(v[18])								// if strategies are fixed
		END_EQUATION(v[1]);					// keep same equation
	...
	if((T-v[2])>=v[3] && v[6]<v[10])		// if time to change
	...
	if(v[0]!=v[1])											// if changed strategy
		WRITE("laststratt0",T);								// updates marker
...
*/
	...
	rank_strat *r=reinterpret_cast<rank_strat *>(SHOOKS(PARENT));
													// get pointer to rank matrix object
	...
	{												// strategies ranks
		...
		r->rsinc[LKH][strat]=r->rsinc[LA][strat]/r->rsinc[KA][strat];	// historic strategy RoI
	}
...
*/
	...
	v[5]=round(V("Tinc"));							// minimum incumbent market time
...
*/
	...
	cur=SEARCH("NetEqTechnologies");			// Pointer to the technologies container
	cur=SHOOKS(cur);							// get top tech pointer
	...
	v[11]=round(V("popmax"));					// maximum number of users
	v[12]=round(V("pop0"));						// minimum number of users
	...
	if(v[5]==T)									// if it is time to innovate
	{
		if(v[13]==1 && v[14]<v[3])				// if incumbent synch applies
			WRITES(cur->up,"tnextrad",T+v[3]-v[14]);
		...
		else
		{
			...
			if(v[17]>v[2])							// applies innovation if better
			{
				...
				v[0]++;								// number of new technology
				cur2=SEARCH("NetEqTechnologies");	// object containing technologies
				WRITE_SHOOKS(cur2,cur1);			// Saves topTech pointer

				WRITES(cur1,"techID",v[0]);			// new ID
				WRITES(cur1,"techt0",T);			// launch: current period
				WRITES(cur1,"techt0incr",T);		// last incremental innovation
				WRITES(cur1,"tnextincr",T+poisson(v[9])); // next incr. innovation time
				WRITES(cur1,"Qmin",floor(v[7]*v[17]/v[2]));	// minimum capacity block
				WRITELLS(cur1,"a",v[17],T-1,1);		// new productivity
				WRITELLS(cur1,"Ptech",1/v[17],T-1,1);	// new price
				WRITELLS(cur1,"cmtech",v[8]*(1/v[17]),T-1,1);	// new maintenance cost
			}

			WRITES(cur->up,"tnextrad",T+poisson(v[3]));
		}											// next radical innovation time
	}
...
*/
	...
	if(v[4]==T)									// if it is time to innovate
	{
		v[7]=5;									// fixed damping factor
		v[8]=v[7]/v[6];							// damping period 2X rad. innov.
		v[9]=T-v[5];							// technology lifetime in avg.
		...
		WRITE("tnextincr",T+poisson(v[2]));		// next incremental innovation time
	}
...
(RECALCS are required to most variables so equations are evaluated also in t)
*/
	...
	cur=SEARCHS(PARENT,"NetEqTechnologies");	// Pointer to the technologies container
	cur=SHOOKS(cur);							// get top tech pointer
	v[8]=VS(cur,"Ptech");						// technology current price
	v[9]=V("pop");								// potential users population
	v[10]=round(V("emax"));						// maximum number of entrants in period
	...
	v[27]=VL("entry",2);
	v[28]=VL("entry",3);
	v[29]=round(V("Temin"));					// minimum time between entries
	v[32]=V("Pstep");							// price change standard step
	v[33]=round(V("popmax"));					// maximum number of users
	v[34]=round(V("pop0"));						// minimum number of users
	v[35]=V("tlastentr");						// last entry time
	v[38]=V("fixstrat");						// define fixed strategies for providers
	...
	if((T-v[35])>=v[29])						// if it is time to try entry
	...
	while(v[20]==1 && v[0]<v[10])				// while entry succeeds (limited to emax)
	{
		...
		{		   								// are unserved users, enter market
			...
			WRITES(cur1,"laststratt0",T);		// last strategy pick period
			...
			WRITES(cur1,"g",999);				// infinite initial revenue growth
			
			WRITELLS(cur1,"Pprov",v[6]*(1-v[32]),T-1,1);// initial price below market average
			WRITELLS(cur1,"Pd",v[6]*(1-v[32]),T-1,1);	// initial desired price
			WRITELLS(cur1,"QM",v[23],T-1,1);	// initial installed capacity
			WRITELLS(cur1,"QP",v[23],T-1,1);	// initial planned capacity
			WRITELLS(cur1,"K",v[23]*v[8],T-1,1);	// initial capital
			WRITELLS(cur1,"I",v[23]*v[8],T-1,1);	// initial investment
			WRITELLS(cur1,"F",v[23]*v[8],T-1,1);	// initial debt = initial investment
			WRITELLS(cur1,"r",v[1],T-1,1);		// start with base interest rate
			WRITELLS(cur1,"M",v[14],T-1,1);		// no user so max quality
			
			RECALCS(cur1,"Kdepr");				// no initial depreciated capital
			RECALCS(cur1,"D");					// no depreciation
			RECALCS(cur1,"L");					// no profit
			RECALCS(cur1,"R");					// no revenue
			RECALCS(cur1,"C");					// no cost
			RECALCS(cur1,"ce");					// no unit cost expectation
			RECALCS(cur1,"CM");					// no maintenance cost
			RECALCS(cur1,"AL");					// no accumulated profit
			RECALCS(cur1,"AF");					// no amortization schedule
			RECALCS(cur1,"NF");					// no finance needs
			RECALCS(cur1,"Q");					// no initial user
			RECALCS(cur1,"s");					// no initial share
			RECALCS(cur1,"gs");					// no initial share growth
			RECALCS(cur1,"group");				// is entrant
			RECALCS(cur1,"negcashper");			// no negative cash period yet
			RECALCS(cur1,"zeroshareper");		// no irrelevant share period yet

			if(!v[38])							// if not using fixed strategies
			...
			WRITE_SHOOKS(cur3,cur);				// saves pointer to technology

			WRITE("tlastentr",T);				// saves time of entry
			v[0]++;								// one more entry
		}
		else
			v[20]=0;
...
*/
	v[1]=round(V("nexit"));						// max # of bad periods

	i=0;										// nothing done yet
	j=0;										// firm count
	k=0;										// exits count

	CYCLE(cur,"Provider")						// count providers before exits
	{											// and the total expected exits
		j++;

		v[2]=VS(cur,"negcashper");				// consecutive negative cash periods
		v[3]=VS(cur,"zeroshareper");			// consecutive irrelevant share per.
		if(v[2]>=v[1] || v[3]>=v[1])			// if max periods reached
			k++;
	}

	if(k==j)									// all firms about to exit?
	{
		v[4]=-1;								// size of largest provider
		CYCLE(cur,"Provider")					// search largest provider
		{
 			v[5]=VS(cur,"Nuser");
 			if(v[5]>v[4])
 			{
 		...
 		WRITES(cur1,"negcashper",0);			// give it more time
		WRITES(cur1,"zeroshareper",0);
	...
	CYCLE(cur,"Provider")						// scans all existing providers
	{
		...
		if(v[2]>=v[1] || v[3]>=v[1])			// if max periods reached
		{
			CYCLES(PARENT,cur1,"User")			// scans all users
			{
				v[7]=VS(cur1,"prov");			// gets user provider

				if(v[7]==v[6])					// check if current provider's user
				{
					WRITES(cur1,"prov",0);		// finishes contract
					WRITES(cur1,"Puser",0);		// so user can search for
					WRITES(cur1,"t0",0);		// a new provider in t+1
					WRITE_SHOOKS(cur1,NULL);
				}
			}

			rp->remove( (unsigned long) v[6] );	// remove from ranking if necessary
...
*/
	...
	CYCLES(GRANDPARENT,cur,"User")				// scans its users
	{
...
*/
	...
	v[0]=0;									// accumulator for GM

	CYCLE(cur,"User")						// scan all users
		if(VS(cur,"effUser"))				// if it's an enabled users
			v[0]+=VS(cur,"B");				// add its budget
...
EQUATION("churning")
/*
Compute global churning for the period
*/

v[1]=V("Nuser");							// get total users in period
v[2]=SUM("churn");							// count total users churning

if(v[1]!=0)
	v[0]=v[2]/v[1];							// compute churning
else
	v[0]=0;
...
Standardized version as an option
*/
	v[0]=v[1]=0;

	CYCLE(cur,"Provider")					// scans all providers
	{
		v[2]=VS(cur,"s");					// get market share
		v[0]+=pow(v[2],2);					// squares sum
		v[1]++;
	}

	if(STD_HHI)								// use standardized version?
		v[0]=max((v[0]-1/v[1])/(1-1/v[1]),0);	// rescale the index
...
EQUATION("C4")
/*
C4 index: joint market share of top 4 firms
*/
	v[1]=V("Nprov");						// total providers

	double *msRank=new double[(int)v[1]];	// vector of market shares

	i=0;
	CYCLE(cur,"Provider")					// scans all providers
	{
		msRank[i]=VS(cur,"s");				// get market share
		i++;
	}

	sort(msRank,msRank+(int)v[1]);		// sort market shares

	for(v[0]=0,j=i-1;j>max(i-5,-1);--j)
		v[0]+=msRank[j];
...
*/
	...
	if(v[1]!=0)							// if there is capital in industry
		CYCLE(cur,"Provider")			// scans all providers
		{
			v[3]=VS(cur,"K");			// get market share
			v[0]+=pow(v[3]/v[1],2);		// squares sum
			v[2]++;
		}

	if(STD_HHI)							// use standardized version?
		v[0]=max((v[0]-1/v[2])/(1-1/v[2]),0);	// rescale the index
...
*/
	...
	{									// debug only
		PLOG("\nStrategies in t=%g\n",T);
		PLOG("Strat(inc):\t1\t2\t3\t4\t5\t6\t7\t8\n");
		PLOG("Nprov:\t%1.0f\t%1.0f\t%1.0f\t%1.0f\t%1.0f\t%1.0f\t%1.0f\t%1.0f\n",
		...
		PLOG("ms(c):\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t\n",
			r->rsinc[SC][1],r->rsinc[SC][2],r->rsinc[SC][3],r->rsinc[SC][4],r->rsinc[SC][5],
			r->rsinc[SC][6],r->rsinc[SC][7],r->rsinc[SC][8]);
		PLOG("RoI(c):\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\n",
			r->rsinc[LKC][1],r->rsinc[LKC][2],r->rsinc[LKC][3],r->rsinc[LKC][4],r->rsinc[LKC][5],
			r->rsinc[LKC][6],r->rsinc[LKC][7],r->rsinc[LKC][8]);
		PLOG("ms(h):\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\n",
			r->rsinc[SH][1],r->rsinc[SH][2],r->rsinc[SH][3],r->rsinc[SH][4],r->rsinc[SH][5],
			r->rsinc[SH][6],r->rsinc[SH][7],r->rsinc[SH][8]);
		PLOG("RoI(h):\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\n",
			r->rsinc[LKH][1],r->rsinc[LKH][2],r->rsinc[LKH][3],r->rsinc[LKH][4],r->rsinc[LKH][5],
			r->rsinc[LKH][6],r->rsinc[LKH][7],r->rsinc[LKH][8]);
		PLOG("Strat(entr):\t11\t12\t13\t14\t15\t16\t17\t18\n");
		PLOG("Nprov:\t%1.0f\t%1.0f\t%1.0f\t%1.0f\t%1.0f\t%1.0f\t%1.0f\t%1.0f\n",
...
delete r;								// free memory allocation
delete rp;								// free memory allocation

if(SAVE_NETW)							// saves the network in a file
{
	char name[256];
	int type;

	type = floor( usernet->cal( NULL, "typeNet", 0 ) );	// get type of network
