...
*/
  ...
  v[6]=V("possible_entry");									
  CYCLE_SAFE( cur, "FIRMS" )							    // use a robust cycle to delete objects
  {
     v[1]=VS(cur, "Market_Share");							//firm's curent market share
     v[2]=VS(cur, "Productive_Capacity");				    //firm's current productive capacity																				
...
*/
   ...
   v[1]=V("Entry_Condition");									//value of entry conditions
   v[6]=SUM("Market_Share");									//sum of firm's market share 
      if(v[1]>0&&v[6]<1)										//if entry conditions are met and there are market space 
      {
      v[20]=V("Effective_Orders_Sector");						//sector effective orders
      ...
      v[34]=VL("Productive_Capacity_Available",1);				//productive capacity available in the last period
	  v[44]=V("Productive_Capacity_Exit");						//productive capacity exited in the current period
      v[36]=V("Avg_Price");										//sector avg price
      v[35]=V("capital_output_ratio");							//sector capital output ratio
      v[38]=V("depreciation");									//sector depreciation period
      v[39]=V("Avg_Productivity_Sector");   					//sector avg productivity	
      ...
      v[5]=COUNT("FIRMS");										//current number of firms
     	...
     	{
     	v[9]=VS(cur4, "Market_Share");							//current firm's market share
		...
		cur5=SEARCH_CND("Market_Share", v[12]);					//search the firm with the closest market share to the average one
		v[2]=VS(cur5, "Market_Share");							//market share of the average firm
		...
		v[7]=v[4]*v[35];										//new firm's number of capitals
			  ...
			  v[41]=V("possible_entry");						//current possible entries
			  WRITE("possible_entry", v[41]-1);					//reduce in 1 possible entries
              ...
              WRITELS(cur, "Market_Share",v[2], t);								//firm's market share
              WRITELS(cur, "Effective_Market_Share",v[2], t);					//firm's effective market share
              WRITELS(cur, "Desired_Market_Share",v[2], t);						//firm's desired market share
              WRITELS(cur, "Firm_Avg_Market_Share",v[2], t);					//firm's avg market share
              WRITELS(cur, "Effective_Orders",(v[2]*v[20]), t);					//firm's effective orders
              WRITELS(cur, "Inventories",(v[2]*v[20]*v[23]), t);				//firm's inventories
              WRITELS(cur, "Price",v[36], t);									//firm's price
              WRITELS(cur, "Max_Capital_Goods_Productivity",v[39], t);			//firm's max capital productivity
              WRITELS(cur, "Frontier_Productivity",v[39], t);					//firm's frontier productivity
			  WRITELS(cur, "Stock_Debt",v[7]*v[25],t);							//firm's stock of debt is the price of capital goods bought
              WRITELS(cur, "Stock_Financial_Assets",0,t);						//firm's stock of financial assets is zero
              ...
              if(v[50]<v[7])													//if the number of capitals of the new firm is still lower than the number of capital it must have
				...
				cur2=ADDNOBJS(cur, "CAPITAL", v[27]);							//create the number of capitals needed
				}
              else
				v[27]=0;
                       
              v[26]=v[37]=0;
              CYCLE_SAFES(cur, cur1, "CAPITAL")									//CYCLE trough firm's capitals
              {
	...
	v[60]=SUM("Market_Share");													//sum of firms market share 
	CYCLE(cur, "FIRMS")															//cycle trough firms to normalize market shares
	{
	v[61]=VS(cur,"Market_Share");												//current firm's market share
	WRITES(cur, "Market_Share", (v[61]/v[60]));									//divide firm's market share by the sum of market shares
	}
   
RESULT(v[4])


EQUATION("Productive_Capacity_Available")
/*
...
*/
	v[0]=V("Productive_Capacity_Exit");
	v[1]=V("Productive_Capacity_Entry");
	v[2]=VL("Productive_Capacity_Available", 1);
	v[3]=v[2]-v[1]+v[0];
RESULT(v[3])
