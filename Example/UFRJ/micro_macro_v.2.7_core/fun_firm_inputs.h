...
*/
	...
	v[1]=VL("Stock_Inputs",1);                   		    //stock of remaining imputs from the last period	
...
*/
	...
	v[5]=VL("Effective_Orders", 1);                   		//firm's effective orders lagged 1 
	v[6]=VL("Effective_Orders", 2);                   		//firm's effective orders lagged 1
	v[7]=V("gama");                                   		//expectation parameter
	...
	v[9]=VL("Stock_Inputs",1);                   			//gives the stock of imputs remaining from the last period
	v[10]=V("Available_Inputs_Ratio");             			//gives the minimium available imput ratio
...
*/
	v[1]=V("Input_Demand_Next_Period");                   //amount of inputs demanded in the current period to be used in the production of the next period                      
	cur=SEARCH_CND("id_intermediate_goods_sector", 1);    //identifies the intermediate goods sector
	v[2]=VS(cur,"Demand_Met_Sector");                     //percentage of the total demand met by the sector                
	v[3]=VS(cur,"Demand_Met_By_Imports");                 //identifies if firms were capable of inporting the amount not mey by the domestic production
	...
	v[6]=V("Effective_Production");                       //firm's effective production   
	v[7]=VL("Stock_Inputs", 1);	                          //firm's stock of inputs in the last period
...
*/
	...
	v[6]=VLS(cur,"Demand_Met_Sector",1);                  //demand for inputs met by the domestic production in the last period
	v[7]=VLS(cur,"Demand_Met_By_Imports", 1);
