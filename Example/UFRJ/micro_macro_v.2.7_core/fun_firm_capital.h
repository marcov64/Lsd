...
*/
	v[0]=V("Investment_Period_Firm");													//if it is investment period for the firm
	if(v[0]==1)
		{
		...
		for (v[3]=0; v[3]<=(v[1]-1); v[3]=v[3]+1)										//for the current production period until the last investment period -1
			{
			v[4]=VLS(cur, "Demand_Met_Sector", v[3]);									//computates the demand met by the sector in the current lag
			v[5]=VLS(cur, "Demand_Met_By_Imports", v[3]);								//computates the demand met by imports in the current lag
			v[6]=VL("Demand_Capital_Goods", v[3]);
...
*/
	...
	v[1]=VL("Frontier_Productivity",v[0]);										//available technology when the capital good was ordered
	v[3]=V("Effective_Productive_Capacity_Variation");							//effective productive capacity variation
	v[4]=V("capital_output_ratio");												//capital output ratio
	v[5]=v[3]*v[4];																//amount of Capital Goods bought
	v[7]=V("Investment_Period_Firm");											//if it is investment period for the firm
	if(v[7]==1 && v[3]>0)
		{
		CYCLE_SAFE(cur, "CAPITAL")												//CYCLE trought Capital for modernization															
			{
		...
		for(v[13]=0; v[13]<=v[5]; v[13]=v[13]+1)								//for the amount of new capital goods bought
			{
			cur=ADDOBJ("CAPITAL");												//create new capital objects
	...
	i=COUNT("CAPITAL");															//count the number of capital goods
