...
EQUATION("Model_Specification")
/*
This variable determines determines model specification, based on switch parameters that rewrites other parameters.
Located at Root.
If the parameter "no_government" is 1, all government parameters are rewritten as 0. If it's not 1, the parameters are those written by the user.
If the parameter "no_external" is 1, all external sector parameters are rewritten as 0. If it's not 1, the parameters are those written by the user.
If the parameter "no_technical" is 1, all R&D parameters are rewritten as 0. If it's not 1, the parameters are those written by the user.
If the parameter "no_quality" is 1, the quality elasticity in all sectors is set to zero, so quality do not affect competitiveness. If it is not 1, the elasticities are those written by the user.
If the parameter "no_autonomous" is 1, the autonomous consumption adjustment parameter is set to 0. If it is not 1, the parameter is the one set by the user.
If the parameter "no_depreciation" is 1, depreciation period paramter is set to 100000 and capital goods date of birth is set to zero. If it is not 1, the parameter is the one set by the user.
If the parameter "no_markup" is 1, desired markup adjustment paramters are set to 0 so desired markup is fixed. If it's not 1, the paramters are those written by the user.
*/
v[0]=V("no_government");
	if (v[0]==1)
		{
		CYCLE(cur, "CLASSES")
			{
			WRITES(cur, "class_direct_tax", 0);
			}
		CYCLE(cur, "SECTORS")
			{
			WRITES(cur, "indirect_tax", 0);
			}
		CYCLE(cur, "GOVERNMENT")
			{
			WRITES(cur, "inflation_sensitivity", 0);
			WRITES(cur, "inflation_target", 0);
			WRITES(cur, "government_surplus_rate_target", 0);
			WRITES(cur, "unemployment_sensitivity", 0);
			WRITES(cur, "unemployment_target", 0);
			WRITES(cur, "interest_rate_adjustment", 0);
			WRITES(cur, "gama_gov", 0);
			}
		}
...
v[1]=V("no_external");		
		...
		CYCLE(cur, "CLASSES")
			{
			WRITES(cur, "class_propensity_to_import", 0);
			}
		CYCLE(cur, "SECTORS")
			{
			WRITES(cur, "exports_coefficient", 0);
			}
		CYCLE(cur, "EXTERNAL_SECTOR")
			{
			WRITES(cur, "exchange_rate_growth", 0);
			WRITES(cur, "external_income_growth", 0);
			}
		}
...
v[2]=V("no_technical");	
	if (v[2]==1)
		...
		CYCLE(cur, "SECTORS")
			{
			WRITES(cur, "imitation_revenue_proportion", 0);
			WRITES(cur, "innovation_revenue_proportion", 0);
			WRITES(cur, "payback", 0);
			WRITES(cur, "passthrough_productivity", 0);
			WRITES(cur, "passthrough_inflation", 0);
			WRITES(cur, "learning_adjustment", 0);
			}
		}
v[3]=V("no_quality");
	if (v[3]==1)
...
v[4]=V("no_autonomous");	
	if (v[4]==1)
		...
		CYCLE(cur, "CLASSES")
			{
			WRITES(cur, "class_autonomous_consumption_adjustment", 0);
			}
		}
v[5]=V("no_depreciation");
	if (v[5]==1)
		{
		CYCLE(cur, "SECTORS")
			{
			WRITES(cur, "depreciation", 10000);
			}
		}	
v[6]=V("no_markup");
if (v[6]==1)		
...
CYCLE(cur, "SECTORS")
{
	v[14]=VS(cur, "investment_period");
	v[15]=VS(cur, "number_object_firms");
	v[17]=V("no_depreciation");					//check the model specification parameter
	if (v[17]!=1)
...
v[25]=VS(cur1, "frontier_productivity_initial");
v[26]=VS(cur2, "frontier_productivity_initial");
v[27]=VS(cur3, "frontier_productivity_initial");
v[28]=VS(cur1, "indirect_tax");
v[29]=VS(cur2, "indirect_tax");
v[30]=VS(cur3, "indirect_tax");
v[31]=VS(cur1, "input_tech_coefficient");
v[32]=VS(cur2, "input_tech_coefficient");
v[33]=VS(cur3, "input_tech_coefficient");
v[34]=VS(cur1, "profits_distribution_rate");
v[35]=VS(cur2, "profits_distribution_rate");
v[36]=VS(cur3, "profits_distribution_rate");
v[37]=VS(cur1, "markup_initial" );
v[38]=VS(cur2, "markup_initial" );
v[39]=VS(cur3, "markup_initial" );
v[40]=VS(cur1, "avg_price_initial");
v[41]=VS(cur2, "avg_price_initial");
v[42]=VS(cur3, "avg_price_initial");
v[43]=VS(cur1, "number_object_firms");
v[44]=VS(cur2, "number_object_firms");
v[45]=VS(cur3, "number_object_firms");
v[46]=VS(cur1, "imitation_revenue_proportion") + VS(cur1, "innovation_revenue_proportion");
v[47]=VS(cur2, "imitation_revenue_proportion") + VS(cur2, "innovation_revenue_proportion");
v[48]=VS(cur3, "imitation_revenue_proportion") + VS(cur3, "innovation_revenue_proportion");

/*****BEGIN CALCULATING INITIAL DEMAND*****/	

	v[19]=SUM("number_object_firms");
...
WRITES(cur1, "demand_initial", v[101]);
WRITES(cur2, "demand_initial", v[102]);
WRITES(cur3, "demand_initial", v[103]);
...
WRITELLS(cur1, "Exports_Sector", v[111], 0, 1);														//write sector 1 initial exports
WRITELLS(cur2, "Exports_Sector", v[112], 0, 1);														//write sector 2 initial exports
WRITELLS(cur3, "Exports_Sector", v[113], 0, 1);														//write sector 3 initial exports
...
{
	v[115]=VLS(cur, "Exports_Sector", 1);															//sector exports
	v[116]=VS(cur, "income_elasticity_exports");													//sector income elasticicty of exports
	v[117]=(v[115]/(pow(v[114], v[116])));															//calculate sector exports coefficient
		WRITES(cur, "exports_coefficient", v[117]);													//write the exports coefficient, assuming external price and foreign price starts as 1, so the exchange rate
}

/*****BEGIN WRITTING GOVERNMENT VARIABLES*****/

cur = SEARCH("GOVERNMENT");																				
WRITES(cur, "total_taxes_initial", v[104]);
...
CYCLE(cur, "SECTORS")
{
	...
	v[1]=VS(cur, "number_object_firms");     
	v[2]=VS(cur, "depreciation");                                                   
	v[3]=VS(cur, "avg_price_initial");                   
	v[4]=VS(cur, "foreign_price_initial");                
	v[6]=VS(cur, "frontier_productivity_initial");
	v[7]=VS(cur, "markup_initial");
	v[8]=VS(cur, "capital_output_ratio");
	v[9]=VS(cur, "desired_inventories_proportion");	
	v[10]=VS(cur, "desired_degree_capacity_utilization");
	v[11]=1/v[1]; 																				//market_share_initial
	v[13]=VS(cur, "demand_initial");															//production_sector_initial (production=demand)
	v[12]=v[13]/v[1]; 																			//production_firm initial
	v[14]=((v[12]*(1+v[9]))/v[10])*v[8];														//number of capital goods of each firm
	v[17]=VS(cur, "input_tech_coefficient"); 	
	v[16]=(v[3]/v[7])-v[17];                        											//sector initial wage
	
	v[18]=VS(cur, "imitation_revenue_proportion") + VS(cur, "innovation_revenue_proportion");
	v[19]=VS(cur, "indirect_tax");
	v[23]=VS(cur, "profits_distribution_rate");

	//Begin Wirtting Sector Independent Variables
		WRITELLS(cur, "Demand_Met_Sector", 1, 0, 1);                              				//it is assumed that all demand is met initially. Equals 1 by definition
		WRITELLS(cur, "Demand_Met_By_Imports", 1, 0, 1);                      					//it is assumed thatt all imports are met initially. Equals 1 by definition
		WRITELLS(cur, "Productive_Capacity_Available", 0, 0, 1);                  				//it is assumed that there is no entry or exit initially. Equals 0 by definition
		WRITELLS(cur, "Avg_Competitiveness_Sector", 1, 0, 1);                     				//if all firms are the same, equals 1 by definition
	//End Writting Sector Independent Variables
	
	//Begin Writting Sector Dependent Variables
		WRITELLS(cur, "Avg_Price", v[3], 0, 1);                                   				//Avg_Price equals avg_price initial
		WRITELLS(cur, "Avg_Price", v[3], 0, 2);													//Avg_Price equals avg_price initial
		WRITELLS(cur, "Foreign_Price", v[4], 0, 1);                               				//Foreign_Price equals foreign_price initial
		WRITELLS(cur, "Avg_Productivity_Sector", v[6], 0,  1);               	 				//If all firms are the same, Avg Productivity will be the initial productivivity for all firms
		WRITELLS(cur, "Max_Productivity_Sector", v[6], 0,  1);                      			//If all capital goods have the same productivity, Max_Productivity equals productivity_initial 
	for (v[101]=1 ; v[101]<=8 ; v[101]=v[101]+1)                        		 				//for 8 lags
		WRITELLS(cur, "Effective_Orders_Sector", v[13], 0, v[101]);               				//Effective_Orders_Sectors equals demand_initial
		WRITELLS(cur, "Inventories_Sector", (v[13]*v[9]), 0, 1);                  				//Firms operate with desired level of inventories, thus, Current stock of inventories is the desired level times effective production
		WRITELLS(cur, "Productive_Capacity_Sector", ((v[13]*(1+v[9]))/v[10]), 0, 1);			//All firms start operating at desired degree of utilization, thus, productive capacity is endogenous calculated based on effective production and desired degree
	...
	cur1=SEARCHS(cur, "FIRMS");																	//search the first and only instance of firms below each sector
	  //Begin Writting Firm Parameters
		...
		WRITELLS(cur1, "Competitiveness", 1, 0, 1);                           					//if all firms are the same
	  	WRITELLS(cur1, "Delivery_Delay", 1, 0, 1);                           		  			//it is assumed that all demand is met initially, so equals 1 by definition
	  for (v[102]=1 ; v[102]<=6 ; v[102]=v[102]+1)                                				//for 6 lags
	  	{
	  	WRITELLS(cur1, "Demand_Productive_Capacity_Replacement", 0, 0, v[102]);					//no replacement initially
	  	WRITELLS(cur1, "Debt_Rate", 0, 0, v[102]);												//no debt initially
	  	}
	  	WRITELLS(cur1, "Stock_Financial_Assets", 0, 0, 1);										//no financial assets initially
	  	WRITELLS(cur1, "Stock_Debt", 0, 0, 1);                                    				//no debt initially
	  ...
	  for(v[122]=1; v[122]<=(v[0]+1); v[122]=v[122]+1)										 	//for (investment period+1) lags (7)
	 		{
	 		WRITELLS(cur1, "Depreciation_Productive_Capacity", 0, 0, v[122]);  					//write 0 
	 		WRITELLS(cur1, "Demand_Productive_Capacity_Expansion", 0, 0, v[122]);     			//write 0 
	 		}
	  ...
	  for (v[107]=1 ; v[107]<=7 ; v[107]=v[107]+1)												//for 7 lags
	  	WRITELLS(cur1, "Market_Share", v[11], 0, v[107]);             							//firm's market share will be the inverse of the number of firms in the sector (initial market share)
	  	WRITELLS(cur1, "Effective_Market_Share", v[11], 0, 1);                    				//firm's effective market share will be the initial market share       
	  	WRITELLS(cur1, "Desired_Market_Share", (v[11]), 0, 1);                  				//firm's desired market share will be twice the initial market share  
	  	WRITELLS(cur1, "Firm_Avg_Market_Share", v[11], 0, 1);                     				//firm's avg market share will be the initial market share
	  	WRITELLS(cur1, "Price", v[3], 0, 1);                                      				//firm's price will be the initial price of the sector, the same for all firms
	  for (v[108]=1 ; v[108]<=6 ; v[108]=v[108]+1)                                				//for 6 lags
	  	{
	  	WRITELLS(cur1, "Frontier_Productivity", v[6], 0, v[108]);                 				//frontier productivity will be the initial frontier productivity
	  	WRITELLS(cur1, "Max_Capital_Goods_Productivity", v[6], 0, v[108]);        				//max capital goods productivity will be the initial frontier productivity that will be the same for all capital goods
	  	...
	  	WRITELLS(cur1, "Avg_Potential_Markup", v[7], 0, 1);										//avg potential markup will be the initial markup
	  	WRITELLS(cur1, "Desired_Markup", v[7], 0, 1); 											//desired markup will be the initial markup
	  for (v[111]=1 ; v[111]<=7 ; v[111]=v[111]+1)                                				//for 7 lags
	  	WRITELLS(cur1, "Potential_Markup", v[7], 0, v[111]);                      				//potential markup will be the initial markup
	  for (v[112]=1 ; v[112]<=12 ; v[112]=v[112]+1)												//for 12 lags
	  	WRITELLS(cur1, "Effective_Orders", v[12], 0, v[112]);                     				//firm's effective orders will be sector's effective orders (given by demand_initial) divided by the number of firms
	  	WRITELLS(cur1, "Sales", v[12], 0, 1);													//firm's sales will be equal to effective orders, no delivery delay
	  	WRITELLS(cur1, "Revenue", (v[12]*v[3]), 0, 1);                            				//firm's revenue will be the firm's sales times firm price
	  	WRITELLS(cur1, "Inventories", (v[12]*v[9]), 0, 1);                        				//firm's inventories will be the sales times the desired inventories proportion (parameter)
	  	WRITELLS(cur1, "Stock_Inputs", (v[17]*v[12]), 0, 1);                      				//firm's stock of imputs will be the sales times the input tech relationship
	  	WRITELLS(cur1, "Productive_Capacity", ((v[12]*(1+v[9]))/v[10]), 0, 1);					//firm productive capacity will be the sales divided by the desired degree of capacity utlization (parameter)
	  	WRITELLS(cur1, "Capital", v[14], 0, 1);													//firm nominal capital equals number of capital if capital goods price equals 1
	  	WRITELLS(cur1, "Wage", v[16], 0, 1); 													//firm's nominal wage equals sector nominal wage initial
			WRITELLS(cur1, "Variable_Cost", (v[16]+v[17]), 0, 1);								//firm variable cost equals unitary wage plus unitary cost of inputs. The last equals the tech coefficient if input price equals 1
				  		
	  		//Begin writting Capital Goods Variables and parameters
	  		cur2=SEARCHS(cur1, "CAPITAL");														//search the first and only instance of capital below firms
	  		WRITELLS(cur2, "Capital_Good_Acumulated_Production", 0, 0, 1);      				//zero by definition
				...
				WRITES(cur2, "id_capital_number", 1);                       					//1 for the first and only initial firm (the others will be created below)
	 			...
	 			CYCLES(cur, cur1, "FIRMS")                                                 		//CYCLE trough all firms
       			...
       			{
       			cur2=SEARCHS(cur1, "CAPITAL");                                         			//search the first and only capital good of each firm
        		cur3=ADDOBJ_EXLS(cur1,"CAPITAL", cur2, 0);			                       		//create new capital goods using the first one as example
        		WRITES(cur3, "id_capital_number", (v[120]+1));									//write the capital good number
        		}
        	CYCLES(cur1, cur5, "CAPITAL")                                            			//CYCLE trough all capital goods
	 					{
	 					v[28]=VS(cur5, "id_capital_number");
        		...
        		v[5]=V("no_depreciation");
        		if(v[5]!=1)																		//if the capital date birth is negative
        			WRITES(cur5, "capital_good_date_birth", v[25]);								//write the capital good date of birth
        		else                                                                   			//if the capital dateof birth is not negative
       		 	  WRITES(cur5, "capital_good_date_birth", 0);                 					//write the date birth minus the depreciation time (it wil only work for 2xDepreciation/InvestmentPeriod capital goods)
        		}
					}
...
cur = SEARCH("GOVERNMENT");
v[41]=VS(cur, "total_taxes_initial"); 															//initial total taxes is calculated in the demand calibration based only on parameters
WRITELLS(cur,"Total_Taxes", v[41], 0, 1);														//write initial total taxes
WRITELLS(cur,"Max_Government_Expenses", v[41], 0, 1);        									//initial max government expenses equals total taxes calculated in the calibration
...
for (v[114]=1 ; v[114]<=4 ; v[114]=v[114]+1)		              								//for 4 lags	
	WRITELLS(cur,"Public_Debt", 0, 0, v[114]);                  								//no debt initially	
v[46]=VS(cur, "interest_rate");																	//base interest rate parameter
WRITELLS(cur,"Interest_Rate", v[46], 0, 1);														//write initial interest rate


cur3 = SEARCH_CND("id_consumption_goods_sector", 1);
v[47]=VS(cur3, "number_object_firms");															//initial total autonomous consumption, 1 good for each consumption goods firm
...
v[48]=SUM("number_object_firms");																//total number of firms
...
CYCLE(cur, "SECTORS")																			//CYCLE trough sectors to write exports coefficient
{
	v[49]=VS(cur, "number_object_firms");														//sector number of firms
	v[45]=v[49]/v[48];																			//sector share of firms
		WRITELLS(cur, "Exports_Sector", (v[37]*v[45]), 0, 1);									//write sector initial exports
	v[43]=VS(cur, "income_elasticity_exports");													//sector income elasticicty of exports
	v[44]=((v[37]*v[45])/(pow(v[42], v[43])));													//calculate sector exports coefficient
		WRITES(cur, "exports_coefficient", v[44]);												//write the exports coefficient, assuming external price and foreign price starts as 1, so the exchange rate
}
