...
*/
	...
	CYCLE_SAFE(cur1, "CAPITALS")
	{
		...
		if(T>=v[5]&&v[4]!=1)								//if the current time step is higher than the date of birth of the capital good plus the depreciation period, this capital good must be depreciated
			{
...
*/
	...
	v[1]=LAG_AVE(THIS,"Firm_Effective_Orders", v[0]);
	v[2]=LAG_AVE(THIS,"Firm_Effective_Orders", v[0], v[0]);
...
*/
	...
	if(v[9]==1)														//if it is investment period for the firm
		...
		CYCLE(cur, "CAPITALS")										//cycle trough all capital goods
		{
			...
			if(T!=v[13] && (T+v[12]) > v[15])						//if current capital good will depreciate in the next investment period (before new capital goods arrive)
 ...
 v[30]=V("switch_interest_investment");
  ...
  SORT("CAPITALS","Capital_Good_Productivity","UP");																								// sort capital goods from the lowest to highest
   CYCLE(cur, "CAPITALS")																																					// CYCLE trought capital goods
   {
     ...
     if(T!=v[19] && T < v[19]-v[1])																					// if the capitalgood was not created in the current period nor will depreciate in the next investment period
     {
