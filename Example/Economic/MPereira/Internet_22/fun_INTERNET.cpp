...
#include "fun_internet.h"				// C++ support functions
...
rank_strat *r;							// global strategy rank matrix
rank_prov_global *rp;					// global provider rank

object *usernet;						// global pointer to users container
...
*/ 
	...
	cur6->hook=SEARCHS(cur6,"Technology");	// saves pointer to top technology
...
*/
	...
	rank_prov_user rank=p;				// create the rank of providers to consider
	...
	p->hook=cur2;						// because price may have changed
...
*/
	...
	cur=SEARCHS(p->up->up,"NetEqTechnologies");	// Pointer to the technologies container
	cur=cur->hook;						// get top tech pointer
...
*/
	...
	v[7]=round(VS(p->up->up,"Ttechdepr"));	// maximum technology lifespan
	...
	if(v[5]==(double)t)							// if it is time to innovate
		...
		else
		{
			...
			if(v[17]>v[2])							// applies innovation if better
			{
				...
				cur2->hook=cur1;					// Saves topTech pointer
				...
				WRITES(cur1,"a",v[17]);				// new productivity
				RECALCS(cur1,"a");
				WRITES(cur1,"Ptech",1/v[17]);		// new price
				RECALCS(cur1,"Ptech");
				WRITES(cur1,"cmtech",v[8]*(1/v[17]));// new maintenance cost
				RECALCS(cur1,"cmtech");
			}
...
*/
	...
	cur=SEARCHS(p->up,"NetEqTechnologies");		// Pointer to the technologies container
	cur=cur->hook;								// get top tech pointer
	...
	while(v[20]==1 && v[0]<v[10])				// while entry succeeds (limited to emax)
	{
		...
		{		   								// are unserved users, enter market
			...
			WRITES(cur1,"Pprov",v[6]*(1-v[32]));// initial price below market average
			RECALCS(cur1,"Pprov");
			WRITES(cur1,"Pd",v[6]*(1-v[32]));	// initial desired price	
			RECALCS(cur1,"Pd");
			WRITES(cur1,"QM",v[23]);			// initial installed capacity
			RECALCS(cur1,"QM");
			WRITES(cur1,"QP",v[23]);			// initial planned capacity
			RECALCS(cur1,"QP");
			WRITES(cur1,"K",v[23]*v[8]);		// initial capital
			RECALCS(cur1,"K");
			WRITES(cur1,"Kdepr",0);				// no initial depreciated capital
			RECALCS(cur1,"Kdepr");
			WRITES(cur1,"I",v[23]*v[8]);		// initial investment 
			RECALCS(cur1,"I");
			WRITES(cur1,"D",0);					// no depreciation
			RECALCS(cur1,"D");
			WRITES(cur1,"F",v[23]*v[8]);		// initial debt = initial investment
			RECALCS(cur1,"F");
			WRITES(cur1,"L",0);					// no profit
			RECALCS(cur1,"L");
			WRITES(cur1,"R",0);					// no revenue
			...
			RECALCS(cur1,"NF");
			WRITES(cur1,"negcashper",0);		// no negative cash period yet
			RECALCS(cur1,"negcashper");
			WRITES(cur1,"zeroshareper",0);		// no irrelevant share period yet
			RECALCS(cur1,"zeroshareper");
			WRITES(cur1,"r",v[1]);				// start with base interest rate
			RECALCS(cur1,"r");
			WRITES(cur1,"M",v[14]);				// no user so max quality
			RECALCS(cur1,"M");
			WRITES(cur1,"Q",0);					// no initial user
			RECALCS(cur1,"Q");
			WRITES(cur1,"s",0);					// no initial share
			...
			RECALCS(cur1,"g");
			WRITES(cur1,"group",0);				// is entrant
			RECALCS(cur1,"group");
			...
			{
				WRITES(cur1,"strat",uniform_int(11,10+STRAT_ENTR));	// picks entrant strategy
				RECALCS(cur1,"strat");
			}
...
*/
	...
	if(k==j)									// all firms about to exit?
	{
		...
		char out[64];
		sprintf(out,"\n Warning: last provider (Nuser=%d) bailed-out at t=%d",(int)v[4],t);
		plog(out);
	}
...
extern char *simul_name;				// configuration name being run
...
void close_sim(void)
{
	...
	if(SAVE_NETW)						// saves the network in a file
	{
		...
		sprintf( name, "%s_%d", simul_name, type + 1 );		// file name to save
		SAVE_NETS( usernet, "User", name );
		sprintf( out, "\nFinal network saved: %s%s%s_%d.net", 
					 path, strlen( path ) == 0 ? "" : "/", name, seed - 1 );
		plog( out );
	}
}
