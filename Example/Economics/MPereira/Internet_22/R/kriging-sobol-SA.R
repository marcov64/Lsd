...
# Database files
folder   <- "data/Sobol"              # data files folder
baseName <- "Facebook"                # data files base name (same as .lsd file)
varName <- "HHIs"    				          # analysis variable name
iniDrop <- 125                        # initial time steps to drop from analysis (0=none)
nKeep <- -1                           # number of time steps to keep (-1=all)


# ==== LSD variables to use (all, the ones to be used in log and new) ====

lsdVars <- c( "Pavg", "Mavg", "VMRP", "popcov", "HHIs", "HHIK",
              "Nprov", "ageavg", "sTentr", "LKTavgentr", "churning", "C4" )
newVars <- c( )
...
dataSet <- read.doe.lsd( folder,                 # data files relative folder
                         ...
                         eval.vars = NULL,       # function to evaluate/adjust/expand the dataset
