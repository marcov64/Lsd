...
To use the 64-bit of Lsd, additionally, you need the Cygwin 64-bit version installed (or a compatible GNU GCC compiler). The installer can be downloaded at http://www.cygwin.com/ (make sure you download the "setup-x86_64.exe" file). Install it to any appropriate directory (the same restrictions mentioned above, about folder names with spaces, also apply to Cygwin). Verify in the installer that "gcc"/"g++" (C++ compiler), "make" (make tool) and "gdb" (debugger) were selected for installation. Alternatively, MinGW gcc-g++ 64-bit can also be used. You don't have to install Tcl/Tk or gnuplot in Cygwin, the required versions are already included with Lsd.
